...
Deno.serve(async (req) => {
	...
	try {
		...
		const { device_id, device_secret, filename, content_type, firmware_version, schema_version } =
			body;
		...
		if (!device_id || !device_secret || !filename || !content_type) {
			return new Response(
				JSON.stringify({
					error: 'Missing required fields: device_id, device_secret, filename, content_type'
				}),
		...
		const { error: uploadRecordError } = await supabase.from('device_uploads').insert({
			device_id: device.id,
