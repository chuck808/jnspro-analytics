...
{#if data.goalProgress && data.goalProgress.length > 0}
  <div class="bg-[#3de8c8]/10 border border-[#3de8c8]/30 rounded-xl p-4 mb-4">
    <div class="flex items-center gap-2 mb-2">
      <svg class="w-5 h-5 text-[#3de8c8]"><!-- target icon --></svg>
    ...
    {#each data.goalProgress as progress}
      <p class="text-xs text-[#9a8f7a]">
        {progress.metric}: {progress.improvement} improvement 
        → {progress.percentToGoal}% to target
      </p>
    {/each}
    <a href="/goals" class="text-xs text-[#3de8c8] hover:underline mt-2 inline-block">
      View all goals →
    </a>
  </div>
{/if}
