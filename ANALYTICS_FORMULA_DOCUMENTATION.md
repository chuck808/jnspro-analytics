...
function scoreEfficiency(curve: SpeedCurve): number {
	...
	const timeToHalfPeak = curve.times.findIndex((t) => curve.speeds[t] >= peakSpeed * 0.5);
...
5. **Front-loaded score:**
   ...
   const frontLoadedScore = Math.max(
   	0,
   	Math.min(100, 100 - ((timeToHalfImpulseS - 0.5) / 0.5) * 50)
   );
   ```
...
4. **Smoothness score (0-100):**
   ...
   const smoothnessScore = Math.max(
   	0,
   	Math.min(100, 100 - (meanAbsolute / (peakAccel * 0.5 || 1)) * 100)
   );
   ```
...
1. **Smooth acceleration data:**
   ...
   ```typescript
   const accelMs2 = smooth(
   	chartData.map((g) => g * GRAVITY),
   	5
   );
   const peakAccel = Math.max(...accelMs2);
...
5. **Calculate transition metrics:**
   ...
   const transitionPhase = {
   	...
   	transitionEfficiency: Math.min(100, (speeds[transitionEnd] / transitionDurationS) * 2)
   };
   ```
