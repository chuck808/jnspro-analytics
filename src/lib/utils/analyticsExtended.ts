...
export function computeJerk(chartData: number[], elapsedMs: number): JerkProfile {
    if (chartData.length < 3) {
        return {
            data: [], peakPositive: 0, peakNegative: 0,
            meanAbsolute: 0, smoothnessScore: 0,
    ...
    const smoothnessScore = Math.max(0, Math.min(100,
        100 - (meanAbs / (peakAccel * 0.5 || 1)) * 100
    ));

    const initialAvg = jerks.slice(0, Math.max(1, Math.floor(jerks.length * 0.1)))
        .reduce((a, b) => a + b, 0) / Math.max(1, Math.floor(jerks.length * 0.1));

    const insight = initialAvg > 0
        ? 'Explosive start — rapid initial force application'
        : 'Gradual power development — smoother initial stroke';

    return {
        data, peakPositive: peakPos, peakNegative: peakNeg,
        meanAbsolute: meanAbs, smoothnessScore,
        initialJerkPositive: initialAvg > 0, insight
    };
}
...
export function computeDetailedPhases(
    ...
    for (let i = driveEnd + 1; i < velGrad.length - 1; i++) {
        if (velGrad[i] < velGrad[i - 1] * 0.8) { inflection = i; break; }
    }

    const driveEff = driveEnd > 0 && peakAccel > 0
        ? velocities[driveEnd] / (driveEnd * dt * peakAccel) : 0;

    const transJerks = jerk.slice(driveEnd, inflection).map(Math.abs);
    const transEff   = transJerks.length > 0 && Math.abs(jerk[driveEnd] || 1) > 0
        ? Math.max(0, 1 - (transJerks.reduce((a, b) => a + b, 0) / transJerks.length) / Math.abs(jerk[driveEnd]))
        : 0.5;

    const velPhaseVels = velocities.slice(inflection);
    const velMaint     = velPhaseVels.length > 0 && peakVel > 0
        ? velPhaseVels.reduce((a, b) => a + b, 0) / (velPhaseVels.length * peakVel) : 0;
...
export function identifyWeaknesses(
    reactionMs:      number,
    techniqueScores: { reaction: number; explosiveness: number; smoothness: number; efficiency: number },
    phaseMetrics:    PhaseMetrics | null,
    jerkProfile:     JerkProfile | null,
    riderLevel:      string | null,
    performanceEngineData?: {
        techniqueAnalysis?: { reaction?: number | null; explosiveness?: number | null; smoothness?: number | null; efficiency?: number | null } | null;
    ...
    const reactionScore = performanceEngineData?.techniqueAnalysis?.reaction ?? techniqueScores.reaction;
    ...
    // ── Explosiveness ─────────────────────────────────────────────────────────
    const explosivenessScore = performanceEngineData?.techniqueAnalysis?.explosiveness ?? techniqueScores.explosiveness;
    ...
    const smoothnessScore = peSmoothnessScore ?? performanceEngineData?.techniqueAnalysis?.smoothness ?? techniqueScores.smoothness;
    ...
    if (phaseMetrics && phaseMetrics.drivePhase.efficiency * 100 < PHASE_EFFICIENCY_THRESHOLD) {
        ...
        weaknesses.push({
            ...
            advice: effScore < effThreshold * 0.7 ? [
    ...
    if (phaseMetrics && phaseMetrics.velocityPhase.maintenanceScore * 100 < velocityThreshold) {
        ...
        weaknesses.push({
            ...
            advice: maintScore < velocityThreshold * 0.75 ? [
...
export function computeSessionStability(
    ...
    results.forEach(r => { r.isBest = r.stability === best; });
    return results;
}
...
export function getStabilityInsight(
    ...
    const avgStability = sessionResults.reduce((sum, r) => sum + r.stability, 0) / sessionResults.length;
