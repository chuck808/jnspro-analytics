...
export function assessDataQuality(biasMs2: number | null): DataQuality {
    if (biasMs2 === null) {
        return { label: 'Poor', color: '#ff4444', badge: '⚠ No data', description: 'Speed analytics unavailable' };
    }
    const abs = Math.abs(biasMs2);
    if (abs < 0.5)  return { label: 'Excellent', color: '#3de8c8', badge: '✓ Excellent', description: 'Highly accurate speed tracking' };
    if (abs < 1.0)  return { label: 'Good',      color: '#f5a623', badge: '✓ Good',      description: 'Reliable speed tracking' };
    if (abs < 2.0)  return { label: 'Fair',       color: '#ffcc44', badge: '⚠ Fair',      description: 'Minor drift — check sensor placement' };
    return           { label: 'Poor',      color: '#ff4444', badge: '⚠ Poor',      description: 'High drift — calibrate before next session' };
}
...
export function scoreTechnique(
    ...
    const benchmarks: Record<string, number> = {
        novice: 350, intermediate: 300, expert: 250, elite: 220
    };
    ...
    const avgJerk = jerkValues.length > 0
        ? jerkValues.reduce((s, j) => s + j, 0) / jerkValues.length
        : 0;
