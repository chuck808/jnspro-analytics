...
import { fitPolynomial, selectBestDegree, predictSessionsRemaining as polyPredict } from './polynomialRegression';
import { fitExponential, isExponentialPattern, predictSessionsRemaining as expPredict } from './exponentialFit';
...
function selectBestCandidate(candidates: SelectedModel[], dataCount: number): SelectedModel {
    ...
    const score = (model: SelectedModel): number => {
        ...
        if (model.sessionsRemaining !== null && model.sessionsRemaining > 0 && model.sessionsRemaining < 50) {
            points += 30;
        }
