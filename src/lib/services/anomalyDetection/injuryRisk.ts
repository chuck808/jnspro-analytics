...
export function assessInjuryRisk(
	...
	const consistencyIssues = fatigueAssessment.indicators.filter(
		(i) => i.type === 'reduced_consistency'
	);
...
function determineRiskLevel(
	...
	if (riskScore >= 70 || hasCritical) {
		return {
			riskLevel: 'critical',
			immediateAction:
				"Rest is strongly advised before your next session. Take 3–5 days of complete rest and speak with your coach. If you're experiencing pain or persistent fatigue, seek a medical review."
		};
	}
	...
	if (riskScore >= 50) {
		return {
			riskLevel: 'high',
			immediateAction:
				'Take 2–3 rest days before your next session. Monitor for unusual fatigue or soreness and flag it with your coach.'
		};
	}
	...
	if (riskScore >= 30) {
		return {
			riskLevel: 'moderate',
			immediateAction:
				'Reduce training intensity by 30-50% for next 1-2 sessions. Focus on recovery.'
		};
	}
...
export function analyzeTrainingPattern(
	...
	const weeklySessionCount = sessions.filter((s) => new Date(s.timestamp) >= oneWeekAgo).length;
	...
	if (sessions.length >= 4) {
		...
		recentIntensitySpike = increase > 0.3; // 30% increase
	}
