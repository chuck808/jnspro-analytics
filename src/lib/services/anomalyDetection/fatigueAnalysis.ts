...
export function analyzeFatigue(
	...
	if (trendIndicator) {
		...
		fatigueScore +=
			trendIndicator.severity === 'critical' ? 40 : trendIndicator.severity === 'warning' ? 25 : 10;
	}
	...
	const baselineSessions = sessions.filter(
		(s) => s.sessionFocus !== 'testing' && s.sessionFocus !== 'technique'
	);
	const consistencyIndicator =
		baselineSessions.length >= 5 ? detectReducedConsistency(baselineSessions, metric) : null;
...
function detectDecliningTrend(
	...
	if (isDeclining) {
		const severity: Severity =
			Math.abs(normalizedSlope) > 0.05
				? 'critical'
				: Math.abs(normalizedSlope) > 0.03
					? 'warning'
					: 'info';
		...
		return {
			...
			recommendation:
				'Performance trending downward - this may indicate accumulated fatigue. Consider a rest day.',
			confidence: 0.7
		};
	}
...
function detectReducedConsistency(
	...
	if (percentIncrease > 30) {
		...
		return {
			...
			recommendation:
				'Increasing variability may indicate fatigue or focus issues. Ensure adequate recovery between sessions.',
			confidence: 0.6
		};
	}
...
function detectTrainingLoad(sessions: SessionData[]): FatigueIndicator | null {
	...
	if (recentSessions.length >= 5) {
		return {
			...
			recommendation:
				"High training frequency detected. Ensure you're allowing adequate recovery time between sessions.",
			confidence: 0.8
		};
	}
...
function detectRecentPerformanceDrop(
	...
	const previousAvg =
		sessions.slice(-4, -1).reduce((sum, s) => sum + s.avgValue, 0) /
		Math.min(3, sessions.length - 1);
...
export function shouldPauseGoals(assessment: FatigueAssessment): boolean {
	return assessment.recommendedAction === 'extended_rest' || assessment.fatigueScore >= 70;
}
