...
export function generateFatigueAlert(assessment: FatigueAssessment): Alert | null {
    ...
    const priority: AlertPriority = 
        assessment.fatigueScore >= 70 ? 'critical' :
        assessment.fatigueScore >= 50 ? 'warning' : 'info';

    const title = 
        assessment.fatigueScore >= 70 ? '🚨 High Fatigue Detected' :
        assessment.fatigueScore >= 50 ? '⚠️ Fatigue Warning' :
        '💡 Fatigue Notice';

    const actionText = 
        assessment.recommendedAction === 'extended_rest' ? 'Take Extended Rest' :
        assessment.recommendedAction === 'rest_day' ? 'Schedule Rest Day' :
        assessment.recommendedAction === 'reduce_intensity' ? 'Reduce Intensity' :
        'Continue Monitoring';
...
export function generateInjuryRiskAlert(assessment: InjuryRiskAssessment): Alert | null {
    ...
    const priority: AlertPriority = 
        ...
        assessment.riskLevel === 'high' ? 'critical' :
        'warning';

    const title = 
        assessment.riskLevel === 'critical' ? '🛑 CRITICAL: Injury Risk' :
        assessment.riskLevel === 'high' ? '🚨 High Injury Risk' :
    ...
    return {
        ...
        priority,
        title,
        message: assessment.immediateAction || `Injury risk factors detected: ${assessment.factors.map(f => f.factor).join(', ')}`,
        actionRequired: true,
...
export function getAlertColor(alert: Alert): string {
    switch (alert.priority) {
        ...
        case 'warning': return '#f5a623';  // Amber
        case 'info': return '#3de8c8';     // Teal
    }
}
...
export function shouldBlockTraining(alerts: Alert[]): boolean {
    return alerts.some(a => 
        a.priority === 'critical' && 
        (a.type === 'injury_risk' || a.type === 'fatigue')
    );
}
