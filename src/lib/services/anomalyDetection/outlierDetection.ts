	...
	for (let i = 0; i < dataPoints.length; i++) {
		...
		if (Math.abs(zScore) > threshold) {
			...
			anomalies.push({
				...
				recommendation: isPerformanceDrop
					? getSuggestionForDrop(metric, Math.abs(zScore))
					: undefined
			});
		}
	}
...
export function detectSuddenChanges(
	...
	for (let i = 1; i < dataPoints.length; i++) {
		...
		if (percentChange > changeThreshold) {
			const isWorse = lowerIsBetter ? curr.value > prev.value : curr.value < prev.value;

			const severity: Severity =
				percentChange > 0.3 ? 'critical' : percentChange > 0.2 ? 'warning' : 'info';
			...
			anomalies.push({
				...
				recommendation: isWorse
					? `Sudden performance drop detected. Review session conditions and recovery status.`
					: undefined
			});
		}
	}
...
export function detectMovingAverageAnomalies(
	...
	for (let i = windowSize; i < dataPoints.length; i++) {
		...
		if (deviation > 0.2) {
			// 20% deviation
			const isWorse = lowerIsBetter ? current.value > movingAvg : current.value < movingAvg;
