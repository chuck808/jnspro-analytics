...
export function performHealthCheck(
    ...
    // 2. Analyze fatigue
    const fatigueAssessment = sessionHistory.length >= 3 
...
function determineOverallStatus(
    ...
    if (injuryRiskAssessment && (injuryRiskAssessment.riskLevel === 'critical' || injuryRiskAssessment.riskLevel === 'high')) {
        return 'critical';
    }
...
function determineShouldRest(
    ...
    if (injuryRiskAssessment && (injuryRiskAssessment.riskLevel === 'high' || injuryRiskAssessment.riskLevel === 'critical')) {
        return true;
    }
    ...
    if (fatigueAssessment && (fatigueAssessment.recommendedAction === 'rest_day' || fatigueAssessment.recommendedAction === 'extended_rest')) {
        return true;
    }
...
export function getStatusColor(healthCheck: TrainingHealthCheck): string {
    switch (healthCheck.overallStatus) {
        case 'healthy': return '#3de8c8';   // Teal
        case 'monitor': return '#f5a623';   // Amber
        case 'caution': return '#ff6b3d';   // Orange
