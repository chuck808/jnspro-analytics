...
export type AdjustmentType = 'increase_target' | 'decrease_target' | 'extend_deadline' | 'shorten_deadline' | 'complete' | 'pause' | 'cancel';
...
function generatePauseSuggestion(context: SuggestionContext): GoalAdjustmentSuggestion {
    return {
        ...
        description: 'Your health check indicates you need rest. Pause this goal temporarily to focus on recovery.',
        currentValue: 'Active',
...
function generateWayBehindSuggestions(context: SuggestionContext): GoalAdjustmentSuggestion[] {
    ...
    suggestions.push({
        ...
        rationale: 'Setting realistic goals maintains motivation. You can always set a new goal after achieving this one.',
        confidence: 0.8,
        autoApply: false
    });
...
function generateStalledSuggestions(context: SuggestionContext): GoalAdjustmentSuggestion[] {
    ...
    suggestions.push({
        ...
        rationale: 'Stalled goals can be demotivating. Either adjust the approach or set a different goal.',
        confidence: 0.7,
        autoApply: false
    });
