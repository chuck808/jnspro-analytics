...
export type ProgressStatus = 'way_ahead' | 'ahead' | 'on_track' | 'behind' | 'way_behind' | 'stalled';
...
export function evaluateGoalProgress(
    ...
    const now = new Date();
    const daysElapsed = Math.max(0, Math.floor((now.getTime() - goal.startDate.getTime()) / (1000 * 60 * 60 * 24)));
    const totalDays = Math.floor((goal.deadline.getTime() - goal.startDate.getTime()) / (1000 * 60 * 60 * 24));
...
function shouldSuggestAdjustment(
    ...
    if (status === 'behind' && daysRemaining < 14 && actualCompletion < 50) {
        return {
            shouldAdjust: true,
            adjustmentReason: 'Limited time remaining - consider adjusting your target to maintain motivation'
        };
    }
...
export function getProgressStatusColor(status: ProgressStatus): string {
    switch (status) {
        case 'way_ahead': return '#3de8c8';   // Teal (excellent)
        case 'ahead': return '#3de8c8';        // Teal (good)
        case 'on_track': return '#f5a623';     // Amber (normal)
        case 'behind': return '#ff6b3d';       // Orange (concerning)
        case 'way_behind': return '#ff4444';   // Red (critical)
        case 'stalled': return '#ff4444';      // Red (critical)
    }
}
...
export function getProgressStatusEmoji(status: ProgressStatus): string {
    switch (status) {
        case 'way_ahead': return '🚀';
        case 'ahead': return '⬆️';
        case 'on_track': return '✅';
        case 'behind': return '⚠️';
        case 'way_behind': return '🐌';
