...
import { generateAdjustmentSuggestions, type GoalAdjustmentSuggestion } from './suggestionGenerator';
...
export function needsAttention(analysis: AdaptiveGoalAnalysis): boolean {
    ...
    if (analysis.progressEvaluation.status === 'way_behind' || 
    ...
    if (analysis.recommendedAction === 'celebrate' && 
...
export function getAdaptationColor(analysis: AdaptiveGoalAnalysis): string {
    ...
    switch (analysis.progressEvaluation.status) {
        case 'way_ahead': return '#3de8c8';
        case 'ahead': return '#3de8c8';
        case 'on_track': return '#f5a623';
        case 'behind': return '#ff6b3d';
        case 'way_behind': return '#ff4444';
        case 'stalled': return '#ff4444';
    }
}
