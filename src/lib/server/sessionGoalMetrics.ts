...
export function computeSessionGoalMetrics(
	...
	const reactions = runs.map((r) => r.reaction_time_ms).filter((v): v is number => v !== null);
	const reactionTime = reactions.length > 0 ? Math.min(...reactions) : null;

	// Max G — measured, higher is better
	const gs = runs.map((r) => r.max_g).filter((v): v is number => v !== null);
	...
	const elapsed = runs.map((r) => r.elapsed_time_ms).filter((v): v is number => v !== null);
