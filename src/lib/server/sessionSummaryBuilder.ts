...
export function buildSessionSummary(session: SessionInput): SessionSummary | null {
	...
	const eligibleRuns = session.runs.filter((r) => !shouldExcludeFromStats(r.tags as any));

	// Flatten gate runs from eligible runs
	const gateRuns: GateRunInput[] = eligibleRuns
		.flatMap((r) => {
	...
	if (reactionTimes.length > 1 && avgReaction !== null && avgReaction > 0) {
		const variance =
			reactionTimes.reduce((s, v) => s + Math.pow(v - avgReaction, 2), 0) / reactionTimes.length;
	...
	const avgMaxG = gValues.length > 0 ? gValues.reduce((s, v) => s + v, 0) / gValues.length : null;
	...
	const avgPeakSpeed = speeds.length > 0 ? speeds.reduce((s, v) => s + v, 0) / speeds.length : null;
	...
	const pitchValues = gateRuns
		...
		.filter((v): v is number => v !== null);
	const avgMaxPitch =
		pitchValues.length > 0 ? pitchValues.reduce((s, v) => s + v, 0) / pitchValues.length : null;
...
export function buildSessionSummaries(sessions: SessionInput[]): SessionSummary[] {
	return sessions.map(buildSessionSummary).filter((s): s is SessionSummary => s !== null);
}
