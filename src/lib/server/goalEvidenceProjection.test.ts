...
describe('projectGoalEvidenceFromSessions', () => {
	it('uses canonical eligibility and reverses when the former PB is excluded', () => {
		...
		const sessions = [
			{ id: 's1', timestamp: '2026-01-02T00:00:00.000Z', runs: [run({ gate_runs: { ...run().gate_runs, reaction_time_ms: 300 } })] },
			{ id: 's2', timestamp: '2026-01-03T00:00:00.000Z', runs: [run({ gate_runs: { ...run().gate_runs, reaction_time_ms: 280 } })] }
		];
	...
	it('tracks peak speed from analytics-valid evidence', () => {
		...
		const sessions = [
			{ id: 's1', timestamp: '2026-01-02T00:00:00.000Z', runs: [run({ gate_runs: { ...run().gate_runs, peak_speed_ms: 8.4 } })] }
		];
	...
	it('records only significant improvements as milestones while retaining the best current value', () => {
		...
		const sessions = [
			{ id: 's1', timestamp: '2026-01-02T00:00:00.000Z', runs: [run({ gate_runs: { ...run().gate_runs, reaction_time_ms: 299 } })] },
			{ id: 's2', timestamp: '2026-01-03T00:00:00.000Z', runs: [run({ gate_runs: { ...run().gate_runs, reaction_time_ms: 298 } })] }
		];
