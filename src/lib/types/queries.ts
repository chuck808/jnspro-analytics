...
export type BikeWithTires = Pick<
	BikeRow,
	'id' | 'name' | 'weight_kg' | 'crank_length_mm' | 'chainring_teeth' | 'sprocket_teeth'
> & {
	front_tire: TireInfo | null;
	rear_tire: TireInfo | null;
};

export type SessionWithRelations = Pick<
	SessionRow,
	| 'id'
	| 'session_type'
	| 'timestamp'
...
> & {
	...
	runs: Array<
		Pick<RunRow, 'id' | 'elapsed_time_ms' | 'distance_m'> & {
			gate_runs: Array<
				Pick<GateRunRow, 'reaction_time_ms' | 'max_g' | 'peak_speed_ms' | 'analytics_valid'>
			> | null;
		}
	>;
};

export type GoalWithProfile = Pick<
	TrainingGoalRow,
	| 'id'
	| 'user_id'
	| 'metric'
