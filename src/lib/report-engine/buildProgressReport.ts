...
export function buildProgressReport(
	...
	const chartsSection = includeCharts
		? createSection({
				...
				priority: 'medium',
				content: [
					'The charts below show whether the patterns described above are stable, improving or fluctuating.'
				],
				charts
			})
		: null;
	...
	const goalsSection = (() => {
		...
		if (earlyStage.length > 0 && detailLevel !== 'simple') {
			earlyStage.forEach((g) => {
				lines.push(`${g.metric} goal set — ${g.percentToGoal ?? 0}% progress so far.`);
			});
		}
	...
	const confidenceSection = includeDiag
		? createSection({
				id: 'data-quality',
				type: 'data-quality',
	...
	return {
		...
		summary: {
			headline,
			confidence,
			body: compactLines([buildOneLinerSummary(input), recommendations[0]?.body ?? null])
		},
		sections,
		charts,
		recommendations
...
function buildProgressExecutiveSummary(
	input: ProgressReportInput,
	...
	if (isSimple(level)) {
		...
		lines.push(
			`Analysis across ${session} session${session === 1 ? '' : 's'}${input.dateRange ? ` (${input.dateRange})` : ''}.`
		);
	}
	...
	if (rxChange !== null && rxChange !== undefined && rxDir) {
		...
		if (isSimple(level)) {
			...
			lines.push(
				`Reaction time: ${rxDir} (${rxChange < 0 ? '−' : '+'}${ms}ms across recent sessions).`
			);
		}
	}
	...
	if (spdChange !== null && spdChange !== undefined) {
		...
		if (isSimple(level)) {
			...
			lines.push(
				`Peak speed: ${spdChange > 0 ? '+' : '−'}${kmh} km/h trend (IMU estimated — directional only).`
			);
		}
	}
...
function buildKeyFindings(input: ProgressReportInput, level: ReportDetailLevel): string[] {
	...
	if (fatigueDir) {
		const improving =
			fatigueDir.toLowerCase().includes('improv') || fatigueDir.toLowerCase().includes('longer');
	...
	if (overallTrend.includes('mixed')) {
		...
		lines.push(
			`   Some metrics are improving while others are declining. Don't push harder — investigate which training variables are changing.`
		);
	} else if (overallTrend.includes('declining')) {
		lines.push(`⚠ Multiple metrics declining`);
		lines.push(
			`   Consider recovery, training load, or technique check. Don't push harder when metrics are going the wrong way.`
		);
	}
...
function buildTrendNarrative(input: ProgressReportInput, level: ReportDetailLevel): string[] {
	...
	if (rxChange !== null && rxChange !== undefined) {
		...
		if (isSimple(level)) {
			...
			lines.push(
				`Reaction time ${rxChange < 0 ? 'improving' : 'declining'} — ${rxChange < 0 ? '−' : '+'}${ms}ms${pct !== null && pct !== undefined ? ` (${Math.abs(pct).toFixed(1)}% ${rxChange < 0 ? 'faster' : 'slower'})` : ''}.`
			);
			if (isCoach(level) && rxChange > 0) {
				lines.push(
					`A rising reaction time trend often signals fatigue, inconsistent pre-gate routine, or increased training load. Check whether other metrics are also declining.`
				);
			}
		}
	}
	...
	if (spdChange !== null && spdChange !== undefined) {
		...
		if (isSimple(level)) {
			...
			lines.push(
				`Peak speed ${spdChange > 0 ? 'trending up' : 'trending down'} ${spdChange > 0 ? '+' : '−'}${kmh} km/h. Note: IMU-estimated speed is directional — treat small changes cautiously.`
			);
		}
	}
	...
	if (gapTrend?.direction) {
		const tightening =
			gapTrend.direction.toLowerCase().includes('improv') ||
		...
		if (isSimple(level)) {
			...
			lines.push(
				`Best vs average gap ${tightening ? 'tightening' : 'widening'} — ${tightening ? 'consistency is improving' : 'floor-raising should be the focus, not peak chasing'}.`
			);
		}
	}
...
function buildFatigueNarrative(input: ProgressReportInput, level: ReportDetailLevel): string[] {
	...
	if (isSimple(level)) {
		if (
			setLengthDir?.toLowerCase().includes('up') ||
			...
			lines.push(
				`You're able to do more quality runs in a row before getting tired — fitness is building.`
			);
		} else if (setLengthDir) {
			lines.push(
				`The number of quality runs before fatigue sets in has been shorter recently. Shorter sets with full recovery between them might help.`
			);
		}
	} else {
		if (setLengthDir) {
			const improving =
				...
				setLengthDir.toLowerCase().includes('improv');
			lines.push(
				`Optimal set length trend: ${setLengthDir}. ${
					improving
...
function buildProgressWatchFor(input: ProgressReportInput, level: ReportDetailLevel): string[] {
	...
	if (isSimple(level)) {
		if (warnings.length > 0) {
			lines.push(
				`In your next few sessions, keep an eye on whether the patterns here continue or change.`
			);
		}
		...
		if (rxChange !== null && rxChange !== undefined && rxChange > 0) {
			lines.push(
				`Reaction time trend — if it continues rising, consider training load, sleep, and pre-gate routine`
			);
		}
		...
		if (spdChange !== null && spdChange !== undefined && spdChange < -1) {
			lines.push(
				`Speed trend — down ${Math.abs(spdChange).toFixed(1)} km/h overall, but check whether individual sessions show variation or a consistent decline`
			);
		}

		if (input.sessionCount < 10) {
			lines.push(
				`Confidence will increase with more sessions — ${10 - input.sessionCount} more needed for full rolling analytics`
			);
		}
	}
...
function buildConfidenceNote(input: ProgressReportInput, level: ReportDetailLevel): string[] {
	...
	if (isSimple(level)) {
		lines.push(
			`This report is based on ${session} sessions. The more sessions you upload, the more reliable the patterns become.`
		);
	} else {
		lines.push(`Confidence: ${conf}. Based on ${session} session${session === 1 ? '' : 's'}.`);
		if (session < 10) {
			lines.push(
				`Full rolling analytics unlock at 10 sessions (${10 - session} more needed). Trends identified now may shift as more data arrives.`
			);
		}
		if (session >= 10) {
			lines.push(
				`Speed values are IMU estimates — directional trends are meaningful but absolute values should be treated with caution.`
			);
		}
		lines.push(
			`Reaction time and G-force are direct measurements and are not affected by estimation uncertainty.`
		);
	}
