...
export function isSimple(level: ReportDetailLevel) { return level === 'simple'; }
export function isCoach(level: ReportDetailLevel)  { return level === 'coach' || level === 'technical'; }
...
export function sessionQualitySummary(
    ...
    if (isSimple(level)) {
        if (q >= 80) lines.push('Your starts were very consistent this session — the quality stayed high across all your runs.');
        else if (q >= 65) lines.push('Most of your runs were good, with a bit of variation between them.');
        else if (q >= 50) lines.push('Some strong runs and some weaker ones — your quality varied quite a bit.');
        else lines.push('Your reaction times and performance varied a lot between runs this session.');
    } else {
        lines.push(`Session quality score: ${q}/100. ${q >= 70 ? 'This reflects consistent execution across the session.' : q >= 50 ? 'Some variability in execution across runs.' : 'Significant variability — performance was not repeatable this session.'}`);
    }

    // Best vs average gap
    if (bestVsAvgGap !== null) {
        if (isSimple(level)) {
            if (bestVsAvgGap < 5) lines.push('Your best run and your average run were very close — that\'s what you want.');
            else if (bestVsAvgGap < 12) lines.push(`Your best run was about ${bestVsAvgGap.toFixed(0)}% better than your average — there\'s some gap to close.`);
            else lines.push(`There\'s a big gap between your best run and your typical run (${bestVsAvgGap.toFixed(0)}%). The focus should be on making the average better, not chasing one perfect start.`);
        } else {
            if (bestVsAvgGap < 5) lines.push(`Best-vs-average gap: ${bestVsAvgGap.toFixed(1)}% — excellent. The rider is executing consistently, not just peak-chasing.`);
            else if (bestVsAvgGap < 12) lines.push(`Best-vs-average gap: ${bestVsAvgGap.toFixed(1)}% — moderate. There\'s meaningful performance left in the typical run.`);
            else lines.push(`Best-vs-average gap: ${bestVsAvgGap.toFixed(1)}% — wide. The rider\'s peak performance significantly exceeds their typical output. Focus should shift to floor-raising over peak-chasing.`);
        }
    }
    ...
    if (optimalSetLength !== null && runCount > 2) {
        if (isSimple(level)) {
            if (optimalSetLength >= runCount) {
                lines.push('Your quality held up across all your runs — no sign of fatigue in this session.');
            } else {
                lines.push(`Your starts were strongest in the first ${optimalSetLength} run${optimalSetLength === 1 ? '' : 's'}. After that, quality started to drop.`);
            }
        } else {
            if (optimalSetLength >= runCount) {
                lines.push(`Set capacity held for all ${runCount} runs — no quality drop-off detected.`);
            } else {
                lines.push(`Optimal set length: ${optimalSetLength} run${optimalSetLength === 1 ? '' : 's'}. Quality begins to deteriorate beyond this point.`);
            }
        }
    }
    ...
    if (dropOffRun !== null && dropOffRun <= runCount) {
        if (isSimple(level)) {
            lines.push(`Run ${dropOffRun} is where performance started to drop off. Ending sets before this point would keep your training quality higher.`);
        } else {
            lines.push(`Drop-off detected at run ${dropOffRun}. Runs beyond this point show >10% decline from session best. Consider ending sets at run ${Math.max(1, dropOffRun - 1)} to preserve quality.`);
        }
    }
...
export function repeatabilitySection(
    ...
    if (isSimple(level)) {
        if (score >= 80) lines.push('Your reaction times were very consistent — you\'re producing reliable starts.');
        else if (score >= 60) lines.push('Your reaction times were reasonably consistent. Some variation between runs, but nothing dramatic.');
        else if (score >= 40) lines.push('Your reaction times varied quite a bit. Working on a consistent pre-gate routine should help.');
        else lines.push('Significant variation in reaction times across the session. Focus on your preparation routine before each start.');
    } else {
        ...
        if (cvPercent !== null) {
            if (cvPercent < 2) lines.push('Outstanding consistency. This level of repeatability under training conditions transfers well to race performance.');
            else if (cvPercent < 5) lines.push('Good consistency. Minor refinement of the pre-gate routine may tighten this further.');
            else if (cvPercent < 10) lines.push('Moderate variation. The preparation state isn\'t fully stable — review pre-gate routine.');
            else lines.push('High variation. This level of inconsistency will produce unpredictable race results. Pre-gate routine needs deliberate work.');
        }
    }
...
export function techniqueSection(
    ...
    if (isSimple(level)) {
        ...
        else if (overall >= 65) lines.push(`Your technique was solid${levelText} with a couple of areas to work on.`);
        else if (overall >= 50) lines.push(`There\'s room to improve your technique${levelText}. The focus areas below are worth working on.`);
        else lines.push(`Your technique needs some work${levelText}. Don\'t be discouraged — these are coachable things.`);
    } else {
    ...
    if (isSimple(level)) {
        if (strongest.score >= 75) lines.push(`Your strongest area is ${strongest.name.toLowerCase()} — keep doing what you\'re doing there.`);
        if (weakest.score < 60) lines.push(`The main area to focus on is ${weakest.name.toLowerCase()} — that\'s where the most improvement is available.`);
    } else {
        ...
        if (highComponents.length > 0 && lowComponents.length > 0) {
            lines.push(`Strong ${highComponents.map(c => c.name.toLowerCase()).join(' and ')}, weaker ${lowComponents.map(c => c.name.toLowerCase()).join(' and ')}. ${techniquePatternInsight(input)}`);
        } else if (lowComponents.length === 0) {
            lines.push(`All technique components above threshold. ${techniquePatternInsight(input)}`);
        } else {
            lines.push(`Multiple technique components below threshold. ${techniquePatternInsight(input)}`);
        }
    }
...
function techniquePatternInsight(input: TechniqueInput): string {
    ...
    if (reaction !== null && efficiency !== null && reaction >= 75 && efficiency >= 75 && explosiveness !== null && explosiveness < 55) {
...
export function dataQualitySection(
    ...
    if (isSimple(level)) {
        if (!analyticsValid) {
            lines.push('Some speed and power figures weren\'t available for this session — the sensor data wasn\'t reliable enough to calculate them. Reaction time and G-force are not affected.');
        } else if (blocksSpeed || blocksPower) {
            lines.push('Speed and power estimates are approximate for this session. Reaction time and G-force remain accurate.');
        }
        return lines;
    }
    ...
    if (biasCorrection !== null) {
        if (biasCorrection < 0.5) {
            lines.push(`Bias correction: ${biasCorrection.toFixed(3)} m/s² — excellent calibration. Integrated speed is reliable.`);
        } else if (biasCorrection < 1.5) {
            lines.push(`Bias correction: ${biasCorrection.toFixed(3)} m/s² — acceptable. Speed estimates carry some uncertainty; use for relative comparison.`);
        } else if (biasCorrection < 3.0) {
            lines.push(`Bias correction: ${biasCorrection.toFixed(3)} m/s² — high. Speed integration is unreliable. Treat speed values as directional indicators only.`);
        } else {
            lines.push(`Bias correction: ${biasCorrection.toFixed(3)} m/s² — outside acceptable range. Speed and power figures have been suppressed.`);
        }
    }

    if (!analyticsValid) {
        lines.push('Firmware analytics were marked invalid for this run. Speed, power, and derived metrics are not shown.');
    } else {
        if (blocksPower) lines.push('Power calculations are blocked by data quality constraints.');
        if (blocksSpeed) lines.push('Speed estimates are suppressed — bias correction exceeds reliable threshold.');
    }

    lines.push('Reaction time and G-force are direct sensor measurements and are not affected by data quality flags.');
...
export function formatRecommendation(
    ...
    if (isSimple(level)) {
        ...
        const priorityLabel = rec.priority === 'high' ? '⚠ Priority' : rec.priority === 'medium' ? '→' : '·';
...
export function watchForSection(
    watchItems: string[],
    level: ReportDetailLevel
): string[] {
    ...
    if (isSimple(level)) {
        return [
            'Next session, look out for:',
            ...watchItems.map((w, i) => `${i + 1}. ${w}`)
        ];
    }
...
export function trendSentence(input: TrendInput, level: ReportDetailLevel): string | null {
    ...
    const changeText = change !== null && change !== undefined && unit
    ...
    if (isSimple(level)) {
        ...
        if (direction === 'declining') return `${metric} is getting worse${changeText} — worth paying attention to.`;
    } else {
...
export function getConfidenceLabel(confidence: 'low' | 'medium' | 'moderate' | 'high' | undefined): string {
...
export function confidenceStatement(
    ...
    if (isSimple(level)) {
        if (confidence === 'high') return `Based on ${sessionCount} sessions — the patterns here are well established.`;
        if (confidence === 'medium') return `Based on ${sessionCount} sessions — the trends are becoming clear but more data will confirm them.`;
        return `Based on only ${sessionCount} session${sessionCount === 1 ? '' : 's'} — these are early signs, not firm conclusions.`;
    }

    if (confidence === 'high') return `High confidence (${sessionCount} sessions). Trend direction is statistically reliable.`;
    if (confidence === 'medium') return `Medium confidence (${sessionCount} sessions). Trend direction is probable but not yet statistically robust.`;
...
export function sessionExecutiveSummary(params: {
    ...
    const { headline, qualityScore: q, bestVsAvgGap, dropOffRun, runCount, dataQualityLabel, level } = params;
    const lines: string[] = [];
    ...
    if (isSimple(level)) {
        ...
        if (dropOffRun !== null && dropOffRun < runCount) lines.push(`Drop-off detected at run ${dropOffRun} of ${runCount}.`);
