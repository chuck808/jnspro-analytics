...
export function buildCoachSessionReport(
	...
	const confidence = narrative?.trust?.confidence ?? sessionReport?.confidence ?? undefined;
	...
	const consistencyBad =
		sessionReport?.repeatability?.overall !== undefined &&
	...
	const metrics: ReportMetric[] = [
		{
			...
			unit: '/100',
			note:
				sessionReport?.sessionQuality !== undefined && sessionReport?.sessionQuality !== null
					? sessionReport.sessionQuality >= 70
						? 'Good'
						: sessionReport.sessionQuality >= 50
							? 'Fair'
							: 'Variable'
					: undefined
		},
		{
			...
			unit: '/100',
			note:
				sessionReport?.repeatability?.cvPercent !== undefined &&
				sessionReport?.repeatability?.cvPercent !== null
					? sessionReport.repeatability.cvPercent < 5
						? 'Excellent'
						: sessionReport.repeatability.cvPercent < 10
							? 'Good'
							: sessionReport.repeatability.cvPercent < 15
								? 'Moderate'
								: 'Variable'
					: undefined
		},
		{
			...
			note:
				...
				sessionReport?.bestVsAvg?.gapPercent !== null
					? sessionReport.bestVsAvg.gapPercent < 5
						? 'Excellent'
						: sessionReport.bestVsAvg.gapPercent < 12
							? 'Moderate'
							: 'Wide'
					: undefined
		},
	...
	const charts: ReportChart[] = includeCharts ? (input.charts ?? defaultSessionCharts()) : [];
	...
	if (isWetSurface) {
		...
		execLines.push(`Surface was damp — treat speed metrics as indicative rather than definitive.`);
	}
	...
	if (weatherCondition === 'windy') {
		...
		execLines.push(`Wet weather — grip-dependent metrics are less reliable today.`);
	}
	...
	if (rideFeel) {
		...
		if (
			(rideFeel === 'peak' || rideFeel === 'dialled') &&
			sessionQuality !== null &&
			sessionQuality < 55
		) {
			execLines.push(
	...
	if (narrativeInsights.length > 0) {
		for (const insight of narrativeInsights) {
			...
			if (detailLevel === 'simple') {
				...
				const tonePrefix =
					insight.tone === 'warning' ? '⚠' : insight.tone === 'positive' ? '✓' : '→';
		...
		if (
			...
			if (gap < 5) {
				...
				findings.push(
					`${gap.toFixed(0)}% gap between best and average run — some variation across the session.`
				);
			} else {
				findings.push(
					`${gap.toFixed(0)}% gap between best and average — performance is not yet consistent.`
				);
			}
		}
		...
		if (
			sessionReport?.setLength?.optimal !== null &&
			sessionReport?.setLength?.optimal !== undefined
		) {
			const optimal = sessionReport.setLength.optimal;
			if (optimal < input.runCount) {
				findings.push(
					`Quality held for approximately ${optimal} run${optimal === 1 ? '' : 's'} before declining.`
				);
			} else {
		...
		if (sessionReport?.phaseAnalysis) {
			...
			if (p.driveEfficiency < 50) {
				...
				findings.push(
					'Drive phase efficiency is strong — initial force application is well-timed.'
				);
			}
		}
	...
	if (
		...
		if (detailLevel === 'simple') {
			...
			crossRunLines.push(
				`Best vs average gap: ${gap.toFixed(1)}% — ${gap < 5 ? 'excellent consistency' : gap < 12 ? 'moderate variation' : 'significant variation, focus on floor-raising'}.`
			);
		}
	}
	...
	if (sessionReport?.phaseAnalysis && detailLevel !== 'simple') {
		const p = sessionReport.phaseAnalysis;
		crossRunLines.push(
			`Drive phase: ${p.driveEfficiency}% efficiency · Transition: ${p.transitionEfficiency}% · Velocity maintenance: ${p.maintenanceScore}%.`
		);
	...
	if (sessionReport?.impulseAnalysis && detailLevel !== 'simple') {
		...
		const frontLoad =
			imp.frontLoadedScore > 0.6
				? 'well front-loaded'
				: imp.frontLoadedScore < 0.4
					? 'back-loaded — consider earlier force application'
					: 'balanced';
		crossRunLines.push(
			`Impulse delivery is ${frontLoad}. 50% of total force reached at ${imp.timeToHalfImpulseS.toFixed(2)}s.`
		);
	}

	const crossRunSection =
		crossRunLines.length > 0
			? createSection({
	...
	const rawWatchItems = [
		...
		sessionReport?.dropOff?.dropOffRun !== null && sessionReport?.dropOff?.dropOffRun !== undefined
	...
	const sections = [
		...
		createSection({
			// 4. Session quality — scores + repeatability
			id: 'session-quality',
			type: 'session-quality',
		...
		...(includeCharts // 7. Charts
			? [
					createSection({
						...
						priority: 'medium',
						content: [
							'Charts support the coaching summary — they show the patterns described above.'
						],
						charts
					})
				]
	...
	const summaryBody = compactLines([summaryImpact ?? execLines[1] ?? null]);
	...
	return {
		...
		summary: {
			headline: finalHeadline,
			confidence:
				typeof confidence === 'string' && ['low', 'medium', 'moderate', 'high'].includes(confidence)
...
function buildWheelieNotes(
	...
	if (isSimple) {
		if (controlledCount > 0 && excessiveCount === 0) {
			lines.push(
				`Front wheel lifted on ${wheelieCount} run${wheelieCount === 1 ? '' : 's'} — controlled and within the useful range.`
			);
		} else if (excessiveCount > 0) {
			lines.push(
				`Front wheel lifted on ${wheelieCount} run${wheelieCount === 1 ? '' : 's'}, with ${excessiveCount} looking excessive. Worth checking with your coach whether this is costing you speed.`
			);
		}
	} else {
		lines.push(
			`Wheelie data: ${wheelieCount} lift${wheelieCount === 1 ? '' : 's'} detected (${controlledCount} controlled, ${excessiveCount} excessive${technique.lateLiftCount ? `, ${technique.lateLiftCount} late` : ''}).`
		);
		if (excessiveCount > 0) {
			lines.push(
				'Excessive front wheel lift typically costs speed in the transition phase. Review timing and body position.'
			);
		}
...
function buildTechnicalAppendix(input: CoachSessionReportInput): string[] {
	...
	if (s) {
		...
		if (s.repeatability?.overall !== undefined)
			lines.push(`Repeatability: ${s.repeatability.overall}/100`);
		if (s.bestVsAvg?.gapPercent !== undefined && s.bestVsAvg?.gapPercent !== null)
			lines.push(`Best-vs-avg gap: ${s.bestVsAvg.gapPercent.toFixed(1)}%`);
		if (s.setLength?.optimal !== undefined)
			lines.push(`Optimal set length: ${s.setLength.optimal} runs`);
 ...
 */
function buildCoachingLogSection(
	input: CoachSessionReportInput
