...
import type { SessionAnalysis, Recommendation, WeaknessAnalysis } from '$lib/performance-engine/types';
...
function combineWeaknesses(
  ...
  if (legacy.phaseMetrics) {
    if (legacy.phaseMetrics.drivePhase.efficiency < 0.70) {
...
function generateLegacyRecommendations(legacy: LegacyMetricsInput): Recommendation[] {
  ...
  if (legacy.speedProfile === 'Late Peak') {
    recommendations.push({
      ...
      message: 'Your speed profile shows a late peak. Focus on explosive first pedal strokes to achieve peak acceleration sooner.',
    ...
    recommendations.push({
      ...
      message: 'Strong early acceleration detected. Work on maintaining power as you transition to top speed.',
      metric: 'speed_profile',
    });
  }
  ...
  if (legacy.jerkProfile) {
    if (legacy.jerkProfile.meanAbsolute > 50) {
      recommendations.push({
        ...
        message: 'High jerk values suggest uneven force application. Focus on smooth, progressive power delivery.',
        metric: 'jerk',
      });
    }
...
export function createUnifiedInsightSummary(enhanced: EnhancedSessionAnalysis): string {
  ...
  if (enhanced.legacyIntegration.combinedWeaknesses.length > 0) {
    parts.push(`Focus Areas: ${enhanced.legacyIntegration.combinedWeaknesses.length} areas identified`);
  }
