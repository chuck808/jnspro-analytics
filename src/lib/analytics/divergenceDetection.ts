...
export function detectDivergence(
    ...
    for (let i = 0; i < minLength; i++) {
        ...
        if (percentDiff > threshold && avg > 0.1) { // Ignore noise near zero
            const severity: 'minor' | 'moderate' | 'major' = 
                percentDiff > 40 ? 'major' :
                percentDiff > 25 ? 'moderate' : 'minor';
    ...
    const avgPercentDiff = divergencePoints.length > 0
    ...
    // Generate insight
    const insight = generateDivergenceInsight(
        divergencePoints,
        similarityScore,
        divergentPhase
    );
...
export function detectMultiRunDivergence(
    ...
    for (let i = 0; i < runs.length; i++) {
        for (let j = i + 1; j < runs.length; j++) {
            const analysis = detectDivergence(
                runs[i].data,
                runs[j].data,
                elapsedMs
            );
    ...
    const consistencyScore = pairwiseAnalysis.length > 0
        ? pairwiseAnalysis.reduce((sum, pa) => sum + pa.analysis.similarityScore, 0) / pairwiseAnalysis.length
        : 100;
