 ...
 */
export function calculateCorrelation(x: number[], y: number[]): { r: number; n: number } | null {
...
export function correlationLanguage(
	...
	const qualifiers: Record<CorrelationResult['strength'], string> = {
		'very strong': smallSample
			? 'worth watching across more sessions'
...
function synthesizeMultiVariableInsights(
	...
	if (
		context.surfaceConsistency &&
		context.runCountConsistency &&
		context.surfaceConsistency.length >= 2
	) {
		...
		if (
			bestSurface.count >= 3 &&
			worstSurface.count >= 3 &&
			context.runCountConsistency.significant
		) {
			const consistencyTrend =
				context.runCountConsistency.direction === 'positive' ? 'decreases' : 'improves';
			const surfaceDiff = (
				((worstSurface.mean - bestSurface.mean) / bestSurface.mean) *
				100
			).toFixed(0);
	...
	if (
		context.tempMaxG &&
		context.tempReaction &&
		context.tempMaxG.significant &&
		context.tempReaction.significant
	) {
		const tempPowerDirection = context.tempMaxG.direction === 'positive' ? 'higher' : 'lower';
		const tempReactionDirection =
			context.tempReaction.direction === 'negative' ? 'faster' : 'slower';
		...
		if (
			(context.tempMaxG.direction === 'positive' &&
				context.tempReaction.direction === 'negative') ||
...
export function generateCorrelationInsights(
	...
	if (tempReaction && tempReaction.significant && Math.abs(tempReaction.correlation) > 0.3) {
		...
		insights.push({
			...
			example:
				tempReaction.direction === 'negative'
	...
	if (
		runCountConsistency &&
		runCountConsistency.significant &&
		Math.abs(runCountConsistency.correlation) > 0.3
	) {
		...
		insights.push({
			...
			priority: 'medium',
			example:
				runCountConsistency.direction === 'positive'
 ...
 */
export function prepareCorrelationData(sessions: any[], sessionStats: any[]): SessionDataPoint[] {
