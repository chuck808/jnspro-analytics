...
export function buildCoachDiagnostics(
	...
	if ((scores.launchQuality ?? 0) >= 80 && (scores.explosiveness ?? 0) < 60) {
		out.push({
			...
			tone: 'warning',
			summary:
				'The rider is reacting well, but the first drive phase is not producing enough acceleration.',
			evidence: [
				`Launch quality ${fmt(scores.launchQuality)}`,
				`Explosiveness ${fmt(scores.explosiveness)}`
			],
			prescription: [
	...
	if ((scores.explosiveness ?? 0) >= 75 && (scores.speedCarry ?? 100) < 60) {
		out.push({
			...
			tone: 'warning',
			summary:
				'The run creates force early, but speed is not being carried through the later part of the effort.',
			evidence: [
				`Explosiveness ${fmt(scores.explosiveness)}`,
				`Speed carry ${fmt(scores.speedCarry)}`
			],
			prescription: [
	...
	if ((scores.smoothness ?? 100) < 60) {
		out.push({
			...
			tone: 'warning',
			summary:
				'The acceleration trace changes sharply, suggesting inconsistent force delivery or noisy sensor mounting.',
			evidence: [
				`Smoothness ${fmt(scores.smoothness)}`,
				p?.jerk?.insight ?? 'Jerk profile needs review'
			],
			prescription: [
	...
	if ((scores.repeatability ?? 100) < 65) {
		...
		if (testing) {
			...
			out.push({
				...
				tone: 'warning',
				summary:
					'The best effort may be useful, but the spread across runs suggests the rider cannot reproduce it consistently yet.',
				evidence: [
					`Repeatability ${fmt(scores.repeatability)}`,
					`Consistency ${analysis.summary.consistencyLabel ?? 'unknown'}`
				],
	...
	if ((scores.impulseTiming ?? 0) >= 80 && p?.impulse) {
		out.push({
			...
			tone: 'positive',
			summary:
				'Most of the estimated force is being produced early enough to support a strong launch.',
			evidence: [
	...
	if (!out.length) {
		out.push({
			...
			tone: 'neutral',
			summary:
				'No single limiter dominates this run. Use the charts to decide whether to chase reaction, force, or consistency next.',
