...
export function generateRecommendations(
	...
	for (const weakness of weaknesses.slice(0, 3)) {
		recs.push({
			priority:
				weakness.score !== null && weakness.score !== undefined && weakness.score < 50
					? 'high'
					: 'medium',
	...
	if (impulse && impulse.timeToHalfImpulseS > 0.8) {
		recs.push({
			...
			message:
				'You are taking a little long to generate half of total impulse; target sharper first-pedal force.',
			metric: 'impulse'
		});
	}
