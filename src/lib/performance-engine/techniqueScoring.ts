import { getBenchmarks, labelScore, scoreAgainstHigherIsBetter, scoreAgainstLowerIsBetter } from './benchmarks';
...
export function scoreRunTechnique(
  ...
  const launchQuality = scoreAgainstLowerIsBetter(run?.reactionMs, b.reactionMs.excellent, b.reactionMs.needsWork);
  const peakG = run?.maxG ?? (physics?.accelerationG.length ? Math.max(...physics.accelerationG.map(Math.abs)) : null);
  ...
  const speedCarry = scoreSpeedCarry(physics?.speedKmh, b.speedCarryRatio.good, b.speedCarryRatio.excellent);
  const smoothness = typeof physics?.jerk?.smoothnessScore === 'number'
...
function scoreSpeedCarry(speeds: number[] | null | undefined, good: number, excellent: number): number | null {
