...
export const IMPACTS = {
	calibrationLimited:
		'Some derived metrics appear unreliable, so coaching should focus on trusted signals only.',
	consistencyHigh:
		'Repeatable performance indicates solid technique and good mental/physical state.',
	consistencyLow:
		'Wide variation between runs suggests inconsistent execution or varying conditions.',
	fewRuns: 'Small sample size limits pattern detection and confidence in conclusions.',
	fatigueDetected:
		'Performance decline within the session suggests energy management or technique breakdown.',
	gapWide:
		'Large gap between best and average suggests occasional breakthrough runs but inconsistent baseline.',
	reactionImproving:
		'Quicker gate reactions enable earlier power application and better overall runs.',
	speedImproving:
		'Higher peak speeds indicate more effective power transfer and technique execution.'
};
...
export const WHY_MATTERS = {
	calibration:
		'Speed and power depend on calibration, time units and integration quality. If those are off, absolute values can mislead.',
	consistency:
		'Consistent runs build confidence and allow progressive training. High variation makes it hard to know what is working.',
	earlyData:
		'Early patterns can guide focus, but should not drive major decisions until confirmed with more sessions.',
	fatigue:
		'Training quality matters more than quantity. If quality drops, further reps may reinforce poor patterns.',
	repeatability:
		'A single great run is less valuable than repeatable solid runs. Consistency builds competition readiness.',
	technique:
		'Observable technique patterns (wheelie control, timing, rhythm) remain valid even when speed/power readings are questionable.'
};
...
export const WATCH_FOR = {
	calibrationImprovement:
		'speed and power values returning to realistic ranges after calibration checks',
...
export function buildRideFeelNote(ctx: RideFeelContext): string | null {
	...
	if (
		(feel === 'peak' || feel === 'dialled') &&
		(consistency === 'variable' || consistency === 'poor')
	) {
	...
	if (
		(feel === 'peak' || feel === 'dialled') &&
		(consistency === 'excellent' || consistency === 'good')
	) {
...
export function buildConditionsNote(
	...
	if (surface === 'wet' || surface === 'muddy') {
		parts.push(
			`Surface was ${surface} — speed and traction-dependent metrics should be compared against other ${surface} sessions only.`
		);
	...
	if (weather === 'windy') {
		parts.push(
			'Windy conditions can affect speed carry and timing — flag if this appears in multiple sessions.'
		);
	} else if (weather === 'cold') {
		parts.push(
			'Cold conditions — reaction time and muscle performance can be affected; compare against similar-temperature sessions.'
		);
	} else if (weather === 'hot') {
		parts.push(
			'Hot conditions — fatigue may accumulate faster than usual; watch drop-off patterns.'
		);
...
export function buildCorrelationNote(
	...
	const condLabel =
		best.variable === 'track_surface' ? surfaceLabel(best.value) : weatherLabel(best.value);
...
export function confidencePhrase(level: 'low' | 'moderate' | 'high'): string {
	switch (level) {
		case 'low':
			return 'Low confidence — early signal only';
		case 'moderate':
			return 'Moderate confidence — pattern emerging';
		case 'high':
			return 'High confidence — clear pattern';
	}
}
