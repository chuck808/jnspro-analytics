...
function analyseVariablePattern(
  ...
  const qualifyingGroups = [...groups.entries()]
    .filter(([, ss]) => ss.length >= MIN_CONDITION_SESSIONS);
  ...
  const segments: ConditionSegment[] = qualifyingGroups.map(([condition, ss]) => ({
    ...
    bestReactionMs: min(ss.map(s => s.bestReactionTimeSec).filter(notNull).map(v => v * 1000)),
    avgReactionMs: avg(ss.map(s => s.avgReactionTimeSec).filter(notNull).map(v => v * 1000)),
  ...
  const strength   = differencePercent >= 15 ? 'strong' : differencePercent >= 8 ? 'moderate' : 'weak';

  const metricLabel = metric === 'reactionTime' ? 'reaction time'
    : metric === 'consistency' ? 'consistency'
    : 'session quality';

  const finding = buildFindingText(variable, metric, best.condition, worst.condition, differencePercent, metricLabel);
 ...
 */
function analyseRideFeelPattern(
  sessions: SessionPerformanceSummary[]
): ContextualPattern | null {
  const FEEL_SCORE: Record<string, number> = {
    off: 1, solid: 2, good: 3, dialled: 4, peak: 5
  };
  ...
  if (highFeel.length < MIN_CONDITION_SESSIONS || lowFeel.length < MIN_CONDITION_SESSIONS) return null;
