 ...
 */
export function applyTruthRulesToReport(report: CrossSessionReport | null): CrossSessionReport | null {
...
export function applyCrossSessionTruthRules(report: CrossSessionReport): TruthRuleResult | null {
  ...
  if (hasDecline) {
    ...
    if (report.performance.reactionTrend.direction === 'down') decliningMetrics.push('reaction time');
  ...
  const hasPositiveHeadline = report.headline.toLowerCase().includes('excellent') ||
  ...
  if (hasWarning && hasPositiveHeadline) {
    return {
      ...
      overrideRecommendations: report.recommendations.filter((r: string) => 
