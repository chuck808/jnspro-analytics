...
function determineOverallTrend(
  ...
  const decliningCount = [
    ...
    performance.reactionTrend.improving === false && performance.reactionTrend.direction !== 'stable',
    consistency.repeatabilityTrend.improving === false && consistency.repeatabilityTrend.direction !== 'stable',
    fatigue.optimalSetLengthTrend.improving === false && fatigue.optimalSetLengthTrend.direction !== 'stable'
  ].filter(Boolean).length;
...
function generateWarnings(
  ...
  if (consistency.bestVsAverageGapTrend.direction === 'up' && !consistency.bestVsAverageGapTrend.improving) {
...
function generateRecommendations(
  ...
  if (!consistency.repeatabilityTrend.improving && consistency.repeatabilityTrend.direction !== 'stable') {
    rec.push('Focus on repeatability over peak runs');
  }

  if (!fatigue.optimalSetLengthTrend.improving && fatigue.optimalSetLengthTrend.direction === 'down') {
