...
export function buildCrossSessionMessaging(report: CrossSessionReport): CrossSessionMessaging {
  if (report.status === 'insufficient-data') {
    return {
      ...
      whyThisMatters: 'Consistent tracking helps identify patterns and optimize your training approach.',
      priority: 'low'
    };
  }
  ...
  const hasConsistencyDecline = !report.consistency.repeatabilityTrend.improving && 
    report.consistency.repeatabilityTrend.direction !== 'stable';
  const hasFatigueIssue = !report.fatigue.optimalSetLengthTrend.improving && 
    report.fatigue.optimalSetLengthTrend.direction === 'down';
  const hasReactionDecline = report.performance.reactionTrend.direction === 'up' && 
  ...
  if (bestImproving && avgDeclining) {
    headline = confidencePrefix + 'Mixed performance — peak runs improving but average performance declining';
    priority = 'high';
    ...
    if (speedChange !== null && speedChange > 0) {
      headline = confidencePrefix + `Speed improving with stable execution (+${speedChange.toFixed(1)} km/h)`;
    } else {
  ...
  const whyThisMatters = buildWhyThisMatters(report, hasSpeedImprovement, hasConsistencyDecline, hasFatigueIssue, bestImproving, avgDeclining);
...
function buildSummary(report: CrossSessionReport): string {
  ...
  } else if (!report.performance.reactionTrend.improving && report.performance.reactionTrend.direction !== 'stable') {
  ...
  } else if (!report.consistency.repeatabilityTrend.improving && report.consistency.repeatabilityTrend.direction !== 'stable') {
  ...
  } else if (!report.fatigue.optimalSetLengthTrend.improving && report.fatigue.optimalSetLengthTrend.direction === 'down') {
 ...
 */
function getConfidencePrefix(confidence: 'low' | 'moderate' | 'high', lookbackSessions: number): string {
