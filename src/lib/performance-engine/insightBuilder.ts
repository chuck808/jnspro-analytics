...
export function buildPerformanceInsightPack(
  ...
  const nextActions = diagnostics.flatMap(d => d.prescription).slice(0, level === 'grom' ? 2 : level === 'rider' ? 3 : 6);
...
function buildHeadline(scores: TechniqueScoreBreakdown, level: DetailLevel): string {
  if (level === 'grom') {
    ...
    if ((scores.repeatability ?? 100) < 65) return 'Good effort — practise making starts repeatable';
    return 'Good data — one clear thing to practise';
  }

  if ((scores.explosiveness ?? 0) >= 75 && (scores.speedCarry ?? 100) < 60) return 'Explosive launch, speed carry needs work';
  if ((scores.launchQuality ?? 0) >= 80 && (scores.explosiveness ?? 0) < 60) return 'Fast reaction, drive force needs work';
...
function buildSummary(scores: TechniqueScoreBreakdown, level: DetailLevel, diagnostics: CoachDiagnostic[]): string {
  if (level === 'grom') {
    if (diagnostics[0]?.tone === 'positive') return 'Nice work. The start is looking strong, so keep practising clean repeats.';
    return 'The start has useful positives. Focus on one small improvement rather than chasing everything at once.';
  }

  if (level === 'coach') {
    return diagnostics.map(d => d.summary).slice(0, 2).join(' ');
  }
  ...
  return bits.length ? `Profile summary: ${bits.join(', ')}.` : 'Technique profile is available, but some source metrics are missing.';
}
