...
export function runPhysicsDiagnostics(
	...
	if (peakSpeed !== null && peakSpeed > thresholds.maxPlausibleSpeedKmh) {
		diagnostics.push({
			...
			suggestion:
				'Check elapsed time units, accelerometer scale, integration drift and whether speed has been converted to km/h more than once.'
		});
		...
		diagnostics.push({
			...
			value: `${round(peakSpeed, 1)} km/h`,
			message:
				'Estimated peak speed is high. It may be valid, but should be checked against known distance/time.',
	...
	if (peakPower !== null && peakPower > thresholds.maxPlausiblePowerW) {
		diagnostics.push({
			...
			suggestion:
				'Power depends on both acceleration and integrated speed. Fix speed calibration before trusting power.'
		});
	}
	...
	if (peakG !== null && peakG > thresholds.maxPlausiblePeakG) {
		diagnostics.push({
			...
			suggestion:
				'Check whether the chart data includes spikes, impacts, raw sensor units, or unfiltered accelerometer magnitude.'
		});
	}
...
export function hasBlockingCalibrationIssue(diagnostics: PerformanceDiagnostic[]): boolean {
	return diagnostics.some(
		(d) => d.severity === 'error' && ['speed', 'power', 'calibration'].includes(d.area)
	);
}
