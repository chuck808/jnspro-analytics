...
export function rowToBenchmarks(row: ThresholdProfileRow): PerformanceBenchmarks {
  return {
    reactionMs:       { excellent: row.reaction_ms_excellent, good: row.reaction_ms_good,     needsWork: row.reaction_ms_needs_work },
    peakG:            { good: row.peak_g_good,                excellent: row.peak_g_excellent },
    timeTo90ImpulseS: { excellent: row.impulse_90_excellent,  good: row.impulse_90_good,       needsWork: row.impulse_90_needs_work },
 ...
 */
export async function getAllBenchmarksFromDB(
  ...
  for (const row of data as ThresholdProfileRow[]) {
    ...
    if (row.profile_level === 'rider' && !data.find((r: ThresholdProfileRow) => r.profile_level === 'intermediate')) {
