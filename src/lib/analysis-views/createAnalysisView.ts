...
export function createAnalysisView(analysis: SessionAnalysis, level: DetailLevel): AnalysisView {
	switch (level) {
		case 'grom':
			return createGromView(analysis);
		case 'rider':
			return createRiderView(analysis);
		case 'elite':
			return createEliteView(analysis);
		case 'coach':
			return createCoachView(analysis);
	}
}
...
function createGromView(analysis: SessionAnalysis): AnalysisView {
	...
	const metrics: DisplayMetric[] = [
		metric(
			'Best reaction',
			formatMs(s.bestReactionMs),
			'How quickly the rider reacted to the gate.',
			'grom'
		),
		metric(
			'Best run',
			s.bestRunNumber ? `Run ${s.bestRunNumber}` : '—',
			'The run with the quickest reaction.',
			'grom'
		),
...
function createRiderView(analysis: SessionAnalysis): AnalysisView {
	...
	const metrics: DisplayMetric[] = [
		metric(
			'Best reaction',
			formatMs(s.bestReactionMs),
			'Fastest reaction in the session.',
			'rider'
		),
		metric(
			'Average reaction',
			formatMs(s.averageReactionMs),
			'Typical reaction across the session.',
			'rider'
		),
		metric('Peak G', formatNumber(s.peakG, 'g'), 'Highest acceleration spike.', 'rider'),
		metric(
			'Peak speed',
			formatNumber(s.peakSpeedKmh, 'km/h'),
			'Estimated highest speed from the acceleration trace.',
			'rider'
		),
		metric(
			'Technique score',
			score(t?.overall),
			'Combined reaction, explosiveness, smoothness and efficiency.',
			'rider'
		)
	];
...
function createEliteView(analysis: SessionAnalysis): AnalysisView {
	...
	const metrics: DisplayMetric[] = [
		...
		metric(
			...
			'Derived from spread in reaction times.',
			'elite'
		),
		metric(
			'Peak G',
			formatNumber(s.peakG, 'g'),
			'Highest acceleration in the selected session.',
			'elite'
		),
		metric(
			'Peak speed',
			analysis.hasCalibrationWarning ? 'Check calibration' : formatNumber(s.peakSpeedKmh, 'km/h'),
			'Integrated from acceleration data.',
			'elite'
		),
		metric(
			'Impulse',
			formatNumber(p?.impulse?.totalImpulseNs, 'N·s'),
			'Estimated force over time using total mass.',
			'elite'
		),
		metric(
			'Time to 90% impulse',
			formatNumber(p?.impulse?.timeToNinetyPctImpulseS, 's'),
			'How quickly the run generated most of its force.',
			'elite'
		),
		metric(
			'Peak power',
			shouldShowPower(analysis.diagnostics)
				? formatNumber(p?.power?.peakW, 'W')
				: 'Calibration needed',
			'Estimated from force × velocity.',
			'elite'
		),
...
function createCoachView(analysis: SessionAnalysis): AnalysisView {
	...
	return {
		...
		headline: buildHeadline(analysis),
		summary:
			'Coach view: same data as elite view, with emphasis on diagnosis, comparison and prescription.',
...
function buildHeadline(analysis: SessionAnalysis): string {
	...
	const hardConditions =
		['rain', 'windy', 'cold', 'hot'].includes(weather ?? '') ||
	...
	if (focus === 'reaction-time') {
		...
		if (consistent && !goodSession)
			return 'Consistent reactions — work now on bringing the average down';
	...
	if (focus === 'explosiveness') {
		const peakG = s.peakG;
		if (peakG != null && peakG >= 2.8 && goodSession)
			return 'Explosive session — peak G numbers are strong';
		if (peakG != null && peakG >= 2.5)
			return 'Good drive through the gate — G-force numbers holding up';
		if (feelingOff && peakG != null && peakG >= 2.3)
			return 'Not the best feeling — but the power was still there';
	...
	if (focus === 'technique') {
		...
		if (technique != null && technique >= 80)
			return 'Technique session clicked — scores are reflecting the work';
		if (technique != null && technique >= 60)
			return 'Technique work in progress — clear areas to refine';
	...
	if (focus === 'endurance') {
		if (!declining && consistent)
			return 'Held it together across the full set — good endurance run';
	...
	if (hardConditions) {
		if (goodSession && consistent)
			return 'Strong session in tough conditions — consistency held up';
		...
		if (!goodSession && feelingOff)
			return 'Tough conditions, off day — but the data is there for next time';
	...
	if (feelingPeak && goodSession) return 'Strong feeling translated into good numbers';
	...
	if (improving && goodSession) return 'Warmed into it — performance improved across the set';

	// Declining — fatigue signal
	if (declining && consistent) return 'Consistent early, tired late — watch the set length';
	...
	if (goodSession && feelingOff) return 'Good numbers on an off-day — that is real progress';
	...
	if (solidSession) return 'Decent session — data collected and clear next steps identified';
...
function buildSimpleHeadline(analysis: SessionAnalysis): string {
	...
	if (s.bestRunNumber != null)
		return `Best start was Run ${s.bestRunNumber} — can you beat it next time?`;
...
function simpleInsights(analysis: SessionAnalysis): DisplayInsight[] {
	...
	if (analysis.summary.consistencyLabel) {
		out.push({
			tone: 'positive',
			title: 'Consistency',
			body: `Starts were ${analysis.summary.consistencyLabel.toLowerCase()}.`
		});
	}
...
function performanceInsights(analysis: SessionAnalysis): DisplayInsight[] {
	const out: DisplayInsight[] = [];
	const t = analysis.selectedRun?.technique;
	if (t?.phaseAssessment)
		out.push({ tone: 'neutral', title: 'Technique', body: t.phaseAssessment });
	const jerk = analysis.selectedRun?.physics?.jerk;
	if (jerk?.insight)
		out.push({
			tone: jerk.smoothnessScore < 60 ? 'warning' : 'positive',
			title: 'Smoothness',
			body: jerk.insight
		});
	...
	if (impulse)
		out.push({
			tone: 'neutral',
			title: 'Impulse',
			body: `90% of impulse was reached at ${impulse.timeToNinetyPctImpulseS}s.`
		});
	return out;
}
