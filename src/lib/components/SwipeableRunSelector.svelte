...
>
	<!-- Swipe indicator -->
	<div class="mb-2 flex items-center justify-between text-xs text-[#6b5f4d]">
		...
		{#each runs as _, i}
			<button
				...
				class="flex min-h-[44px] min-w-[44px] items-center justify-center
                    rounded focus:ring-2 focus:ring-[#f5a623] focus:ring-offset-2 focus:ring-offset-[color:var(--theme-surface)] focus:outline-none"
				aria-label="Go to run {i + 1}"
				aria-current={i === selectedIdx ? 'true' : undefined}
			>
				<span
					class="block rounded-full transition-all
                            {i === selectedIdx ? 'h-2 w-6 bg-[#f5a623]' : 'h-2 w-2 bg-[#221c18]'}"
				>
				</span>
			</button>
		{/each}
