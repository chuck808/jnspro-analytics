<script lang="ts">
	...
	function calculateRanking(
		...
		const status: 'pb' | 'excellent' | 'good' | 'average' = isPB
			? 'pb'
			: percentOfBest >= 95
				? 'excellent'
				: percentOfBest >= 85
	...
	function compareToRecent(
		...
		if (percentDiff > 2) {
			// Only consider meaningful differences
			if (lowerIsBetter) {
...
<div class="rounded-xl border border-[#221c18] bg-[#131010] p-5">
	<div class="mb-4">
		<h3 class="mb-1 text-sm font-semibold text-[#f0ece4]">📊 Historical Context</h3>
	...
	{#if historical}
		<div class="space-y-3">
			{#each rankings as ranking}
				...
				{@const trend = compareToRecent(
					ranking.current,
					ranking.recentAverage,
					ranking.lowerIsBetter
				)}
				...
				<div class="rounded-lg border border-[#221c18] bg-[#0a0809] p-4">
					<div class="mb-3 flex items-start justify-between">
						<div class="flex-1">
							<p class="mb-1 text-sm font-semibold text-[#f0ece4]">{ranking.metric}</p>
							{#if rank}
								<div class="flex items-center gap-2">
									<span
										class="rounded px-2 py-0.5 text-xs font-bold"
										style="background:{statusConfig[rank.status].color}20; color:{statusConfig[
											rank.status
										].color}"
									>
										{statusConfig[rank.status].icon}
										{statusConfig[rank.status].label}
									</span>
									{#if !rank.isPB}
					...
					<div class="grid grid-cols-3 gap-3 text-xs">
						<div>
							<p class="mb-1 text-[#6b5f4d]">Current</p>
							<p class="font-mono font-bold text-[#f0ece4]">
								{ranking.current !== null
									? ranking.formatValue(ranking.current)
									: '—'}{ranking.unit}
							</p>
						</div>
						<div>
							<p class="mb-1 text-[#6b5f4d]">All-Time Best</p>
							<p class="font-mono font-bold text-[#f5a623]">
								{ranking.allTimeBest !== null
									? ranking.formatValue(ranking.allTimeBest)
									: '—'}{ranking.unit}
							</p>
						</div>
						{#if ranking.recentAverage !== null}
							<div>
								<p class="mb-1 text-[#6b5f4d]">Recent Avg</p>
					...
					{#if trend && trend.trend !== 'stable'}
						<div class="mt-3 border-t border-[#221c18] pt-3">
							<div class="flex items-center gap-2">
								<span class="text-xs font-semibold" style="color:{trendConfig[trend.trend].color}">
									{trendConfig[trend.trend].icon}
									{trendConfig[trend.trend].label}
								</span>
								<span class="text-xs text-[#6b5f4d]">
		...
		<div class="mt-4 rounded-lg border border-[#221c18] bg-[#0a0809] p-3">
			<p class="text-xs text-[#9a8f7a]">
				💡 <strong class="text-[#f0ece4]">Note:</strong> Recent average is calculated from your last 5
				sessions. Keep training consistently to see improvement trends!
			</p>
		</div>
	{:else}
		<div class="py-8 text-center">
			<p class="mb-2 text-sm text-[#6b5f4d]">No historical data available yet</p>
			<p class="text-xs text-[#9a8f7a]">
