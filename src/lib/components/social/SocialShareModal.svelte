<script lang="ts">
    ...
    function handleLinkedIn() {
        window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(HOMEPAGE_URL)}`, '_blank', 'width=550,height=500');
    }

    function handleReddit() {
        window.open(`https://www.reddit.com/submit?url=${encodeURIComponent(HOMEPAGE_URL)}&title=${encodeURIComponent(buildCaption())}`, '_blank', 'width=850,height=600');
    }

    function handleFacebook() {
        window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(HOMEPAGE_URL)}`, '_blank', 'width=550,height=420');
    }
...
>
    <div class="modal-sheet">
        ...
        <div class="card-preview-wrapper" bind:this={cardPreviewWrapper}>
            <div class="card-scale-container" style="transform: scale({cardScale}); transform-origin: top left;">
                <SocialCard
                    {achievement}
                    {backgroundImageUrl}
                    {profileIconUrl}
        ...
        <div class="share-actions">
            ...
            <button class="share-btn download-btn" onclick={handleDownload} title="Download as PNG" disabled={downloading}>
                <div class="share-icon">
                    {#if downloading}
        ...
        <p class="download-note">Download saves a PNG at 2× resolution. Share buttons post a link to AppGatePro.</p>
...
<style>
    ...
    .download-btn .share-icon { background: rgba(245, 166, 35, 0.15); color: #f5a623; }
    .download-btn { border-color: rgba(245, 166, 35, 0.2); }
