...
<section class="space-y-4">
	<header class="rounded-xl border border-[#221c18] bg-[#131010] p-5">
		<p class="text-xs tracking-wide text-[#f5a623] uppercase">Training Insights</p>
	...
	{#if showSessionSection && sessionReport}
		<section class="space-y-3">
			<div class="flex items-center justify-between gap-3">
				<div>
					<p class="text-xs tracking-wide text-[#f5a623] uppercase">Session Analysis</p>
					<h3 class="text-lg font-semibold text-[#f0ece4]">{sessionTitle}</h3>
				</div>
				<span
					class="rounded-full border border-[#221c18] bg-[#221c18] px-3 py-1 text-xs text-[#9a8f7a]"
				>
					single session
				</span>
			</div>
	...
	{#if showTechniqueSection && hasTechniqueData}
		<section class="space-y-3">
			<div class="flex items-center justify-between gap-3">
				<div>
					<p class="text-xs tracking-wide text-[#f5a623] uppercase">Technique Layer</p>
	...
	{#if showProgressSection}
		<section class="space-y-3">
			<div class="flex items-center justify-between gap-3">
				<div>
					<p class="text-xs tracking-wide text-[#3de8c8] uppercase">v8.1</p>
			...
			{:else}
				<div class="rounded-xl border border-[#221c18] bg-[#131010] p-5">
					...
					<p class="mt-1 text-sm text-[#9a8f7a]">
						Keep logging sessions. Cross-session intelligence appears once enough session summaries
						are available.
					</p>
				</div>
			{/if}
	...
	{#if showCoachDetail && isAdvanced}
		<section class="rounded-xl border border-[#221c18] bg-[#131010] p-5">
			<p class="text-xs tracking-wide text-[#f5a623] uppercase">Coach framing</p>
			<h3 class="mt-1 text-lg font-semibold text-[#f0ece4]">How to read this page</h3>

			<div class="mt-3 grid gap-3 md:grid-cols-2">
				<div class="rounded-xl border border-[#221c18] bg-[#0a0809] p-3">
					<h4 class="font-semibold text-[#f5a623]">This Session</h4>
					<p class="mt-1 text-sm text-[#9a8f7a]">
						Use this to decide whether today's set length, repeatability, fatigue, and execution
						were useful.
					</p>
				</div>
