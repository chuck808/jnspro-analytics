<script lang="ts">
  ...
  const qualityColor = $derived(
    report.sessionQuality >= 80 ? '#3de8c8' :
    report.sessionQuality >= 60 ? '#f5a623' :
    '#ff4444'
  );

  const fatigueColor = $derived(
    report.fatigue.trend === 'improving' ? '#3de8c8' :
    report.fatigue.trend === 'stable' ? '#f5a623' :
    report.fatigue.trend === 'declining' ? '#ff4444' :
    'var(--theme-text-subtle)'
  );

  const fatigueLabel = $derived(
    report.fatigue.trend === 'improving' ? 'Improving ↗' :
    report.fatigue.trend === 'stable' ? 'Stable →' :
    report.fatigue.trend === 'declining' ? 'Declining ↘' :
    'Unknown'
  );
</script>
...
<section class="intelligence-panel">
  ...
  <div class="metrics-grid">
    ...
    <div class="metric-card">
      ...
      <span class="metric-value" style="color: {report.repeatability.overall >= 70 ? '#3de8c8' : '#f5a623'}">
    ...
    <div class="best-vs-avg-card">
      ...
      <div class="comparison-grid">
        ...
        <div>
          ...
          <span class="value" style="color: {report.bestVsAvg.consistencyType === 'consistent' ? '#3de8c8' : report.bestVsAvg.consistencyType === 'moderate' ? '#f5a623' : '#ff4444'}">
    ...
    <div class="advanced-grid">
      ...
      {#if report.fatigue.diff !== undefined}
        <div>
          ...
          <span class="value">{report.fatigue.diff > 0 ? '+' : ''}{report.fatigue.diff.toFixed(2)} km/h</span>
        </div>
      {/if}
    </div>
  </details>
