<script lang="ts">
	...
	const dataQuality = $derived(assessDataQuality(runs[0]?.gate_runs?.bias_correction_ms2));
	...
	const wheeliePattern = $derived(() => {
		...
		if (excessiveCount > runs.length * 0.3) {
			return {
				...
				advice:
					'You might benefit from focusing on weight distribution and smoother power delivery',
	...
	const qualityColor = $derived(
		dataQuality.rating === 'excellent'
			? '#3de8c8'
			: dataQuality.rating === 'good'
				? '#82c91e'
				: dataQuality.rating === 'fair'
					? '#f5a623'
					: dataQuality.rating === 'calibrate'
						? '#ff4444'
						: 'var(--theme-text-subtle)'
	);

	const wheelieColor = $derived(
		wheeliePattern().pattern === 'controlled'
			? '#3de8c8'
			: wheeliePattern().pattern === 'late'
				? '#f5a623'
				: '#ff4444'
	);
...
<section class="technique-panel">
	...
	{:else}
		...
		<div class="metrics-grid">
			...
			<div class="metric-card">
				...
				<span class="metric-hint"
					>{wheelieAnalysis.filter((w) => w.analysis.detected).length}/{runs.length} detected</span
				>
			</div>
			...
			{#if phaseConsistency.length > 0}
				<div class="metric-card">
					...
					<span
						class="metric-value"
						style="color: {phaseConsistency[0].consistency >= 80 ? '#3de8c8' : '#f5a623'}"
					>
		...
		{#if runs.length >= 2}
			<PerformanceDistributionChart {runs} height={compact ? 'h-48' : 'h-64'} {compact} />
		{/if}
						...
						<div class="phase-card">
							...
							<div class="phase-stats">
								...
								<div class="stat">
									...
									<span
										class="value"
										style="color: {phase.consistency >= 80
											? '#3de8c8'
											: phase.consistency >= 60
												? '#f5a623'
												: '#ff4444'}"
									>
