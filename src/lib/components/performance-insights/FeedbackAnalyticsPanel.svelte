<script lang="ts">
	...
	const confusionRate = $derived(
		report.overall.total > 0 ? (report.overall.confusing / report.overall.total) * 100 : 0
	);
	...
	const healthStatus = $derived.by(() => {
		...
		if (overallScore >= 60)
			return { label: 'Needs Attention', color: 'var(--color-jns-amber, #f5a623)' };
...
<section class="feedback-analytics-panel">
	...
	<div class="metrics-grid">
		...
		<div class="metric-card">
			...
			<p class="metric-hint">
				{((report.overall.useful / report.overall.total) * 100).toFixed(0)}% of total
			</p>
		</div>
		<div class="metric-card">
			...
			<div class="problem-list">
				{#each problemInsights as item}
					<div class="problem-item">
						...
						<span
							class="problem-score"
							style="color: {item.usefulnessScore < 40
								? '#ff4444'
								: 'var(--color-jns-amber, #f5a623)'}"
						>
			...
			<div class="insight-section">
				...
				{#each report.byInsightType as item}
					<div class="insight-row">
						...
						<span
							class="insight-score"
							style="color: {item.usefulnessScore >= 75
								? 'var(--color-jns-mint, #3de8c8)'
								: item.usefulnessScore >= 60
