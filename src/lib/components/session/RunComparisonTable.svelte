...
<div class="themed-card rounded-xl p-5">
	<div class="mb-4">
		<h3 class="themed-text-primary mb-2 text-base font-bold">Run Comparison Matrix</h3>
		...
		<table class="w-full text-xs">
					...
					{#each columns as col}
						<th class="px-2 pb-2 text-left">
							<button
								...
								class="themed-text-secondary hover:themed-text-primary group flex items-center gap-1 font-semibold transition-colors"
							>
			...
			<tbody>
				{#each sortedRuns as run}
					<tr
						class="border-b border-[color:var(--border)]/50 transition-colors hover:bg-[color:var(--card-nested)]"
					>
						<td class="themed-text-primary px-2 py-2 font-mono font-semibold">
							{run.runNumber}
						</td>
							...
							<td class="px-2 py-2">
								{#if value !== null}
									<div class="flex items-center gap-1">
										<span class="font-semibold" style="color: {getScoreColor(value)};">
											{value.toFixed(0)}
										</span>
										{#if best}
											<span class="text-base text-[#3de8c8]">🏆</span>
										{:else if worst}
											<span class="text-base text-[#ff6b3d]">⬇️</span>
										{/if}
									</div>
								{:else}
	...
	<div
		class="themed-text-subtle mt-4 flex items-center justify-between border-t border-[color:var(--border)] pt-4 text-xs"
	>
