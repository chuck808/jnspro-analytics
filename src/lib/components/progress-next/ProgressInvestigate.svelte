				...
				<article data-tone={signal.latestTone}>
					...
					<div class="signal-copy">
						<div class="title-row">
							...
							<span class="audience-badge">{signal.latestAudience}</span>
						</div>
						...
						<details class="signal-details">
							<summary>Evidence &amp; next step</summary>
							<p class="signal-summary">{signal.latestSummary}</p>
							{#if signal.latestEvidence.length > 0}
								<ul class="signal-evidence">
									{#each signal.latestEvidence as line}
										<li>{line}</li>
									{/each}
								</ul>
							{/if}
							{#if signal.latestPrescription.length > 0}
								<ol class="signal-prescription">
									{#each signal.latestPrescription as step}
										<li>{step}</li>
									{/each}
								</ol>
							{/if}
...
<style>
	...
	.title-row { display:flex; align-items:center; gap:.4rem; min-width:0; }
	.signal-copy strong { display:block; overflow:hidden; color:#e8f2fb; font-size:.66rem; text-overflow:ellipsis; white-space:nowrap; }
	.audience-badge { flex:0 0 auto; border:1px solid #29465d; border-radius:999px; padding:.1rem .4rem; color:#8298aa; font-size: 0.55rem; font-weight:700; text-transform:uppercase; letter-spacing:.06em; }
	.signal-copy > span { display:block; margin-top:.15rem; color:#70879b; font-size: 0.55rem; }
	...
	.signal-details { margin-top:.5rem; }
	.signal-details summary { cursor:pointer; color:#9cb1c3; font-size: 0.55rem; font-weight:700; }
	.signal-summary { margin:.4rem 0 0; color:#a6b7c5; font-size:.58rem; line-height:1.5; }
	.signal-evidence, .signal-prescription { margin:.35rem 0 0; padding-left:1.1rem; color:#8298aa; font-size:.55rem; line-height:1.55; }
	.signal-evidence li, .signal-prescription li { margin-top:.15rem; }
	article a { border:1px solid #29465d; border-radius:.48rem; padding:.35rem .48rem; color:#9cb1c3; font-size: 0.55rem; text-decoration:none; }
