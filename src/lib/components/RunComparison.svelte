...
<div class="rounded-xl border border-[#221c18] bg-[#131010] p-5">
	<div class="mb-4">
		<h3 class="mb-1 text-sm font-semibold text-[#f0ece4]">🔍 Run Comparison</h3>
	...
	<div class="mb-4 grid grid-cols-2 gap-3">
		<div>
			<label for="run1-select" class="mb-2 block text-xs font-medium text-[#9a8f7a]">Run 1</label>
			<select
				id="run1-select"
				bind:value={selectedRun1}
				class="w-full rounded-lg border border-[#221c18] bg-[#0a0809] px-3 py-2 text-sm text-[#f0ece4]
                       focus:border-transparent focus:ring-2 focus:ring-[#f5a623] focus:outline-none"
			>
		...
		<div>
			<label for="run2-select" class="mb-2 block text-xs font-medium text-[#9a8f7a]">Run 2</label>
			<select
				id="run2-select"
	...
	{#if run1 && run2}
		<!-- Summary -->
		{#if summary}
			<div class="mb-4 rounded-lg border border-[#221c18] bg-[#0a0809] p-3">
		...
		<div class="overflow-x-auto">
			<table class="w-full text-sm">
				<caption class="sr-only"
					>Comparison between Run {run1.runNumber} and Run {run2.runNumber}</caption
				>
				<thead>
					<tr class="border-b border-[#221c18]">
						<th
							scope="col"
							class="pr-4 pb-3 text-left text-xs font-semibold tracking-wider text-[#9a8f7a] uppercase"
						>
							Metric
						</th>
						<th
							scope="col"
							class="pr-4 pb-3 text-right text-xs font-semibold tracking-wider text-[#9a8f7a] uppercase"
						>
							Run {run1.runNumber}
						</th>
						<th
							scope="col"
							class="pr-4 pb-3 text-center text-xs font-semibold tracking-wider text-[#9a8f7a] uppercase"
						>
							Δ
						</th>
						<th
							scope="col"
							class="pb-3 text-right text-xs font-semibold tracking-wider text-[#9a8f7a] uppercase"
						>
							Run {run2.runNumber}
						</th>
					</tr>
				...
				<tbody>
					{#each metrics as metric}
						<tr class="border-b border-[#221c18]/50 transition-colors hover:bg-[#171210]">
							<td class="py-3 pr-4 font-medium text-[#f0ece4]">{metric.label}</td>
							<td
								class="py-3 pr-4 text-right font-mono {metric.diff?.better === 'run1'
									? 'text-[#3de8c8]'
									: 'text-[#9a8f7a]'}"
							>
								{metric.run1}
							...
							<td class="py-3 pr-4 text-center">
								{#if metric.diff}
									<span
										class="rounded px-2 py-0.5 text-xs {metric.diff.better !== 'equal'
							...
							<td
								class="py-3 text-right font-mono {metric.diff?.better === 'run2'
									? 'text-[#3de8c8]'
									: 'text-[#9a8f7a]'}"
							>
								{metric.run2}
