<script lang="ts">
	...
	let { sessionId, notes = [], currentUserRole = 'rider' }: Props = $props();
...
<div class="overflow-hidden rounded-xl border border-[#221c18] bg-[#131010]">
	...
	<div class="border-b border-[#221c18] px-5 py-4">
		<div class="flex items-center justify-between gap-4">
			...
			{#if !isAdding && !editingNoteId}
				<button
					onclick={startAdd}
					class="flex items-center gap-2 rounded-lg bg-[#f5a623] px-3 py-1.5
                           text-xs font-semibold text-[#0a0809] transition-colors hover:bg-[#c97e0a]
	...
	<!-- Tabs -->
	<div class="flex gap-1 overflow-x-auto border-b border-[#221c18] bg-[#0a0809] px-5 py-3">
		{#each Object.entries(NOTE_TYPE_CONFIG) as [type, config]}
			...
			<button
				...
				class="flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium
                       ...
                       focus:ring-2 focus:ring-[#f5a623] focus:outline-none
                       {activeTab === noteType
					? 'border border-[#221c18] bg-[#131010] text-[#f0ece4]'
					: 'text-[#9a8f7a] hover:bg-[color:var(--theme-surface)]/50 hover:text-[#f0ece4]'}"
			>
				<span>{config.icon}</span>
				<span>{config.label}</span>
				{#if count > 0}
					<span class="rounded bg-[#221c18] px-1.5 py-0.5 text-[10px]">{count}</span>
				{/if}
			</button>
		{/each}
	</div>
				...
				<div class="rounded-xl border border-[#221c18] bg-[#0a0809] p-4">
					...
					<div class="mt-3 flex items-center gap-2">
						<button
							...
							class="rounded-lg bg-[#f5a623] px-4 py-2 text-sm font-semibold
						...
						<button
							type="button"
							onclick={cancelAdd}
							class="rounded px-4 py-2 text-sm text-[#9a8f7a] transition-colors
		...
		<div class="space-y-3">
			{#each notesByType[activeTab] as note}
				{#if editingNoteId === note.id}
					...
					>
						...
						<div class="rounded-xl border border-[#f5a623]/30 bg-[#0a0809] p-4">
							<RichTextEditor
							...
							<div class="mt-3 flex items-center gap-2">
								<button
									type="submit"
									class="rounded-lg bg-[#f5a623] px-4 py-2
				...
				{:else}
					<!-- Display note -->
					<div
						class="rounded-xl border border-[#221c18] bg-[#0a0809] p-4 transition-colors hover:border-[#221c18]/80"
					>
						<div class="mb-3 flex items-start justify-between gap-3">
							...
							<div class="flex items-center gap-1">
								<button
									...
									class="rounded p-1.5 text-[#9a8f7a] transition-colors hover:bg-[color:var(--theme-surface)]
                                           hover:text-[#f0ece4] focus:ring-2 focus:ring-[#f5a623] focus:outline-none"
									title="Edit note"
								>
								...
								<form method="POST" action="?/deleteNote" use:enhance>
									...
									<button
										...
										class="rounded p-1.5 text-[#9a8f7a] transition-colors hover:bg-[color:var(--theme-surface)]
                                               hover:text-[#ff4444] focus:ring-2 focus:ring-[#ff4444] focus:outline-none"
										title="Delete note"
									>
										<svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
											<path
						...
						<div class="prose prose-sm tiptap-content max-w-none">
			...
			{#if notesByType[activeTab].length === 0 && !isAdding}
				<div class="py-8 text-center">
					<div
						class="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-[#221c18]"
					>
						<span class="text-2xl">{NOTE_TYPE_CONFIG[activeTab].icon}</span>
					</div>
					<p class="mb-1 text-sm font-medium text-[#9a8f7a]">
						No {NOTE_TYPE_CONFIG[activeTab].label} notes yet
					</p>
					<p class="mb-3 text-xs text-[#6b5f4d]">{NOTE_TYPE_CONFIG[activeTab].description}</p>
					<button
						onclick={startAdd}
						class="inline-flex items-center gap-2 rounded-lg border border-[#f5a623]/20 bg-[#f5a623]/10
