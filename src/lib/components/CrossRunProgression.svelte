...
<div class="themed-card rounded-xl p-5">
	<div class="mb-4 flex flex-wrap items-center justify-between gap-3">
		<div>
			<h3 class="themed-text-primary mb-1 text-sm font-semibold">📊 Cross-Run Progression</h3>
			<p class="themed-text-subtle text-xs">
				Track performance trends across all runs in this session
			</p>
		</div>
			...
			{#each metrics as metric}
				<button
					...
					class="rounded-lg border px-3 py-1.5 text-xs transition-colors
						...
						: 'themed-nested-card themed-text-secondary border-[color:var(--theme-border)] hover:border-[#f5a623]/20'}"
				>
					{metric.label}
				</button>
			{/each}
	...
	{#if trend}
		<div
			class="themed-nested-card mt-4 flex items-start gap-3 rounded-lg border border-[color:var(--theme-border)] p-3"
		>
			<svg
				class="mt-0.5 h-4 w-4 flex-shrink-0 {trend.improving ? 'text-[#3de8c8]' : 'text-[#f5a623]'}"
				fill="none"
				stroke="currentColor"
				viewBox="0 0 24 24"
			>
			...
			<div class="flex-1">
				<p
					class="text-xs font-semibold {trend.improving
						? 'text-[#3de8c8]'
						: 'text-[#f5a623]'} mb-0.5"
				>
					{trend.improving ? '✓ Improving' : '⚠ Degrading'}
				...
				<p class="themed-text-secondary text-xs">
					{currentMetric.label}
					{trend.improving ? 'improved' : 'degraded'} by {trend.percentChange.toFixed(1)}% from
					first to last run
