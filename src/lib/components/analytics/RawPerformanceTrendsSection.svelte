<script lang="ts">
	...
	let overallConsistency = $derived(
		scoreConsistency(sessions.map((s) => s.avg_reaction_ms).filter((v): v is number => v !== null))
	);
				...
				options: {
					...
					scales: {
						...
						y: {
							...baseOpts.scales?.y,
							reverse: true,
							title: { display: !isMobile, text: 'Reaction time (s)', color: darkTick }
						}
					}
				} as any
			});
			...
			const c = new Chart(speedChartEl, {
				...
				options: {
					...
					scales: {
						...
						y: {
							...baseOpts.scales?.y,
							title: { display: !isMobile, text: 'Speed (km/h)', color: darkTick }
						}
					}
				} as any
			});
			...
			const c = new Chart(consistencyEl, {
				...
				options: {
					...
					scales: {
						...
						y: {
							...baseOpts.scales?.y,
							title: { display: !isMobile, text: 'CV % (lower = more consistent)', color: darkTick }
						}
					}
				} as any
			});
			...
			<div class="mb-1 flex items-start justify-between gap-4">
				<div class="flex items-center gap-2">
					<div>
						...
						<p class="mt-0.5 text-xs text-[#6b5f4d]">Lower is better · Y-axis reversed</p>
					</div>
					<HelpButton onclick={() => onOpenHelp('reactionTrend')} />
				</div>
				{#if trend.reaction !== null}
					{@const t = trendArrow(trend.reaction, true)}
					<span class="flex-shrink-0 text-sm font-bold" style="color:{t.color}"
						>{t.icon} {t.text}</span
					>
				{/if}
			</div>
			...
			<div class="mb-1 flex items-start justify-between gap-4">
				<div class="flex items-center gap-2">
					<div>
						...
						<p class="mt-0.5 text-xs text-[#6b5f4d]">⚠ Estimated from IMU · Higher is better</p>
					</div>
		...
		<div class="mb-4 flex items-start justify-between gap-4">
			<div class="flex items-center gap-2">
				<div>
					...
					<p class="mt-0.5 text-xs text-[#6b5f4d]">
						CV % — lower = more consistent within a session
					</p>
				</div>
			...
			{#if overallConsistency}
				<div class="flex-shrink-0 text-right">
					<p class="text-lg font-bold" style="color:{overallConsistency.color}">
						{overallConsistency.label}
					</p>
