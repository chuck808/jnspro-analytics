<script lang="ts">
    ...
    let { sessionCount, crossSessionReport, latestSessionRatings, personalBests, isMobile }: Props = $props();
    ...
    function getConfidenceBadge(level: string) {
        switch (level) {
            case 'high': return { label: 'High Confidence', color: '#3de8c8', bg: '#3de8c820' };
            case 'medium': return { label: 'Medium Confidence', color: '#f5a623', bg: '#f5a62320' };
            case 'low': return { label: 'Building Baseline', color: '#9a8f7a', bg: '#9a8f7a20' };
            default: return { label: 'Unknown', color: '#6b5f4d', bg: '#6b5f4d20' };
        }
    }
</script>
...
<div class="space-y-5">
    ...
    <div class="bg-[#131010] border border-[#221c18] rounded-xl p-6">
        {#if headline}
            ...
            <div class="flex items-start justify-between gap-4 mb-4">
                <div class="flex-1">
                    <p class="text-xs uppercase tracking-wide text-[#6b5f4d] mb-2">Performance Status</p>
                    <h2 class="text-2xl font-bold mb-2" style="color:{headline.color}">{headline.text}</h2>
                </div>
                <div class="rounded-full px-3 py-1.5 text-xs font-medium" style="background:{badge.bg}; color:{badge.color}">
                    {badge.label}
                </div>
            </div>
        {/if}
        ...
        {#if topRecommendations.length > 0}
            <div class="mt-4 pt-4 border-t border-[#221c18]">
                <p class="text-xs font-semibold text-[#f5a623] uppercase tracking-wider mb-2">Recommended Actions</p>
                <ul class="space-y-2">
                    {#each topRecommendations as rec}
                        <li class="flex items-start gap-2 text-sm text-[#f0ece4]">
                            <span class="text-[#3de8c8] mt-0.5">→</span>
                            <span>{rec}</span>
                        </li>
                    {/each}
                </ul>
        ...
        {#if sessionQualityData.length >= 3}
            <div class="mt-4 pt-4 border-t border-[#221c18]">
                <p class="text-xs font-semibold text-[#6b5f4d] uppercase tracking-wider mb-3">Session Quality Trend</p>
                <div class="flex items-end gap-1 h-16">
                    {#each sessionQualityData.slice(-10) as point}
                        <div class="flex-1 flex flex-col items-center gap-1">
                            <div class="w-full rounded-t transition-all" 
                                 style="height:{(point.quality / 10) * 100}%; background:{point.quality >= 7 ? '#3de8c8' : point.quality >= 5 ? '#f5a623' : '#ff4444'}">
                            </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {#each [
            { label:'Best Reaction', value:fmtReaction(personalBests.reaction_ms), sub:'all time', ratingKey: 'reactionTimeSec' },
            { label:'Best Peak Speed', value:fmtSpeed(personalBests.peak_speed_ms), sub:'estimated · all time', ratingKey: 'peakSpeedKmh' },
            { label:'Best Max G', value:fmtG(personalBests.max_g), sub:'all time', ratingKey: 'peakG' },
        ] as pb}
            ...
            <div class="bg-[#131010] border border-[#221c18] rounded-xl p-5">
                <div class="flex items-center justify-between mb-2">
                    <p class="text-xs text-[#6b5f4d] uppercase tracking-wider">{pb.label}</p>
                    {#if rating}
                        <span class="text-xs px-2 py-0.5 rounded-full font-medium"
                              style="background:{getRatingColor(rating.rating)}20; color:{getRatingColor(rating.rating)}">
                            {getRatingLabel(rating.rating)}
                        </span>
                    {/if}
                </div>
                <p class="text-3xl font-bold text-[#f5a623]">{pb.value}</p>
                <p class="text-xs text-[#6b5f4d] mt-1">{pb.sub}</p>
            </div>
        {/each}
    </div>
</div>
