      ...
      options: {
        ...
        scales: {
          ...
          y: {
            ...
            title: { display: !isMobile, text: 'Smoothness Score', color: '#9a8f7a' }
          }
        }
      }
    });
...
<div class="bg-[#131010] border border-[#221c18] rounded-xl p-5">
  <div class="flex items-start justify-between gap-4 mb-4">
    <div>
      ...
      <p class="text-xs text-[#6b5f4d] mt-1">How cleanly you apply power through the start</p>
    </div>
    {#if trend}
      <div class="text-right">
        <div class="text-xs text-[#6b5f4d]">Current</div>
        <div class="text-2xl font-bold" style="color:{
          trend.direction === 'smoother' ? '#3de8c8' :
          trend.direction === 'rougher' ? '#ff4444' : '#f5a623'
        }">{trend.current}/100</div>
        <div class="text-xs {
          trend.direction === 'smoother' ? 'text-[#3de8c8]' :
          trend.direction === 'rougher' ? 'text-[#ff4444]' : 'text-[#9a8f7a]'
        }">
    ...
    <div class="mt-3 space-y-2">
      ...
      <div class="grid grid-cols-2 gap-2 text-xs">
        <div class="p-2 bg-[#0a0809] rounded border border-[#221c18]">
          <div class="text-[#6b5f4d] mb-1">What is smoothness?</div>
          <div class="text-[#9a8f7a]">Lower "jerk" = more fluid technique</div>
        </div>
        <div class="p-2 bg-[#0a0809] rounded border border-[#221c18]">
          <div class="text-[#6b5f4d] mb-1">Why it matters</div>
