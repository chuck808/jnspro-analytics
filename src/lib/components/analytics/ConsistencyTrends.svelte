<script lang="ts">
    ...
    let improvements = $derived.by(() => {
        ...
        return {
            repeatability: first.repeatabilityScore && last.repeatabilityScore
                ? ((last.repeatabilityScore - first.repeatabilityScore) / first.repeatabilityScore) * 100
                : null,
            cv: first.cvPercent && last.cvPercent
                ? ((first.cvPercent - last.cvPercent) / first.cvPercent) * 100 // Lower is better
                : null,
            bestVsAvgGap: first.bestVsAvgGap && last.bestVsAvgGap
...
<div class="themed-card rounded-xl p-5">
    ...
    {#if improvements && data.length >= 3}
        <div class="grid grid-cols-3 gap-3 p-4 themed-nested-card rounded-lg">
            <div class="text-center">
                ...
                {:else}
                    <p class="text-sm themed-text-subtle">—</p>
                {/if}
            </div>
            
            <div class="text-center">
                <p class="text-xs themed-text-subtle mb-1">CV Improvement</p>
            ...
            <div class="text-center">
                <p class="text-xs themed-text-subtle mb-1">Gap Reduction</p>
        ...
        <div class="mt-3 text-xs themed-text-subtle text-center">
