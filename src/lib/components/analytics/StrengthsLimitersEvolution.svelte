...
<div class="themed-card rounded-xl p-5">
	<div class="mb-4">
		<h3 class="themed-text-primary mb-2 text-base font-bold">Strengths & Limiters Evolution</h3>
	...
	{:else}
		<div class="space-y-4">
			...
			{#if evolution.consistentStrengths.length > 0}
				<div>
					<h4
						class="mb-2 flex items-center gap-2 text-xs font-semibold tracking-wider text-[#3de8c8] uppercase"
					>
						<span>💪</span>
					...
					<div class="flex flex-wrap gap-2">
						{#each evolution.consistentStrengths as strength}
							<div
								class="inline-flex items-center gap-2 rounded-full border border-[#3de8c8]/30 bg-[#3de8c8]/10
                                       px-3 py-1.5 text-sm text-[#3de8c8]"
							>
								<span>{strength.name}</span>
								<span class="text-xs opacity-70"
									>({strength.frequency}/{sessionInsights.length})</span
								>
							</div>
						{/each}
					</div>
			...
			{#if evolution.emergingStrengths.length > 0}
				<div>
					<h4
						class="mb-2 flex items-center gap-2 text-xs font-semibold tracking-wider text-[#4a9eff] uppercase"
					>
						<span>🌱</span>
					...
					<div class="flex flex-wrap gap-2">
						{#each evolution.emergingStrengths as strength}
							<div
								class="inline-flex items-center gap-2 rounded-full border border-[#4a9eff]/30 bg-[#4a9eff]/10
                                       px-3 py-1.5 text-sm text-[#4a9eff]"
							>
								{strength}
							</div>
						{/each}
				...
				<div>
					...
					<div class="flex flex-wrap gap-2">
						{#each evolution.resolvedLimiters as limiter}
							<div
								class="inline-flex items-center gap-2 rounded-full border border-[#3de8c8]/30 bg-[#3de8c8]/10
                                       px-3 py-1.5 text-sm text-[#3de8c8] line-through opacity-70"
							>
								{limiter}
							</div>
						{/each}
			...
			{#if evolution.persistentLimiters.length > 0}
				<div>
					<h4
						class="mb-2 flex items-center gap-2 text-xs font-semibold tracking-wider text-[#ff6b3d] uppercase"
					>
						<span>🎯</span>
					...
					<div class="flex flex-wrap gap-2">
						{#each evolution.persistentLimiters as limiter}
							<div
								class="inline-flex items-center gap-2 rounded-full border border-[#ff6b3d]/30 bg-[#ff6b3d]/10
                                       px-3 py-1.5 text-sm text-[#ff6b3d]"
							>
								<span>{limiter.name}</span>
								<span class="text-xs opacity-70"
									>({limiter.frequency}/{sessionInsights.length})</span
								>
							</div>
						{/each}
					</div>
