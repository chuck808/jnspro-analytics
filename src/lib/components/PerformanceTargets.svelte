<script lang="ts">
	...
	function calculateProgress(
		...
		const status: 'met' | 'close' | 'needs-work' =
			percent >= 100 ? 'met' : percent >= 90 ? 'close' : 'needs-work';
...
<div class="rounded-xl border border-[#221c18] bg-[#131010] p-5">
	<div class="mb-4">
		<h3 class="mb-1 text-sm font-semibold text-[#f0ece4]">🎯 Performance Targets</h3>
			...
			<div class="rounded-lg border border-[#221c18] bg-[#0a0809] p-4">
				<div class="mb-2 flex items-start justify-between">
					<div class="flex-1">
						<div class="mb-1 flex items-center gap-2">
							...
							{#if progress}
								<span
									...
									style="background:{statusColors[progress.status].bg}20; color:{statusColors[
										progress.status
									].text}"
								>
									{statusColors[progress.status].icon}
									{progress.status === 'met'
										? 'Target Met!'
						...
						<p class="mb-3 text-xs text-[#6b5f4d]">{target.description}</p>
					</div>
				</div>
				...
				{#if progress}
					<div class="mb-2">
						<div class="h-2 w-full overflow-hidden rounded-full bg-[#221c18]">
							<div
				...
				<div class="flex items-center justify-between text-xs">
					<div>
						...
						<span class="ml-1 font-mono font-bold text-[#f0ece4]">
							{target.current !== null
								? `${target.current.toFixed(target.unit === 'ms' ? 0 : 1)}${target.unit}`
								: '—'}
						</span>
					</div>
					<div>
					...
					{#if progress && progress.gap !== 0}
						<div>
							...
							<span
								...
								style="color:{progress.status === 'met'
									? statusColors.met.text
									: statusColors['needs-work'].text}"
							>
								{progress.status === 'met' ? '-' : target.lowerIsBetter ? '+' : ''}{Math.abs(
									progress.gap
								).toFixed(target.unit === 'ms' ? 0 : 1)}{target.unit}
							</span>
						</div>
					{/if}
				</div>
	...
	<div class="mt-4 rounded-lg border border-[#221c18] bg-[#0a0809] p-3">
		<p class="text-xs text-[#9a8f7a]">
			💡 <strong class="text-[#f0ece4]">Tip:</strong> Targets are based on typical performance for {riderLevel}
			level riders. Adjust your profile settings to see personalized goals.
		</p>
	</div>
</div>
