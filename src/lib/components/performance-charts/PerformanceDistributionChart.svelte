<script lang="ts">
    ...
    const metricConfig = {
        reaction: {
            label: 'Reaction Time',
            unit: 's',
            getValue: (run: any) => run.gate_runs?.reaction_time_ms ? run.gate_runs.reaction_time_ms / 1000 : null,
            color: '#f5a623',
            decimals: 3,
            lowerIsBetter: true,
        },
        speed: {
            label: 'Peak Speed',
            unit: 'km/h',
            getValue: (run: any) => run.gate_runs?.peak_speed_ms ? run.gate_runs.peak_speed_ms * 3.6 : null,
            color: '#ff6b3d',
            decimals: 1,
            lowerIsBetter: false,
        },
        ...
        chart = new Chart(chartCanvas, {
            ...
            data: {
                ...
                datasets: [
                    ...
                    {
                        label: '+1σ',
                        data: annotatedPoints.map((_, i) => ({ x: i, y: (stats?.mean ?? 0) + (stats?.stdDev ?? 0) })),
...
<div class="bg-[#131010] border border-[#221c18] rounded-xl p-5">
    ...
    <div class="flex items-center justify-between mb-4 flex-wrap gap-3">
        ...
        <div class="flex items-center gap-1 bg-[#0a0809] rounded-lg p-1 border border-[#221c18]">
            {#each [
                ...
                { value: 'gforce', label: 'G-Force', icon: '💪' },
                { value: 'elapsed', label: 'Time', icon: '⏱️' }
            ] as option}
                <button
    ...
    {#if dataPoints.length === 0}
        <div class="bg-[#0a0809] rounded-lg p-6 border border-[#221c18] text-center">
            <p class="text-sm text-[#6b5f4d]">No {metricConfig[metric].label.toLowerCase()} data available for these runs.</p>
        </div>
    {:else}
        <!-- Chart -->
        ...
        {#if stats}
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                <div class="bg-[#0a0809] rounded-lg p-3 border border-[#221c18]">
                    <p class="text-xs text-[#6b5f4d] mb-1">Average</p>
                    <p class="text-lg font-bold text-[#f0ece4]">{fmt(stats.mean, metricConfig[metric].decimals)}<span class="text-xs text-[#9a8f7a] ml-1">{metricConfig[metric].unit}</span></p>
                </div>
                
                <div class="bg-[#0a0809] rounded-lg p-3 border border-[#221c18]">
                    <p class="text-xs text-[#6b5f4d] mb-1">Best</p>
                    <p class="text-lg font-bold text-[#3de8c8]">{fmt(metricConfig[metric].lowerIsBetter ? stats.min : stats.max, metricConfig[metric].decimals)}<span class="text-xs text-[#9a8f7a] ml-1">{metricConfig[metric].unit}</span></p>
                </div>
                
                <div class="bg-[#0a0809] rounded-lg p-3 border border-[#221c18]">
                    <p class="text-xs text-[#6b5f4d] mb-1">Spread</p>
                    <p class="text-lg font-bold text-[#f0ece4]">±{fmt(stats.stdDev, metricConfig[metric].decimals)}<span class="text-xs text-[#9a8f7a] ml-1">{metricConfig[metric].unit}</span></p>
                </div>
                
                <div class="bg-[#0a0809] rounded-lg p-3 border border-[#221c18]">
                    <p class="text-xs text-[#6b5f4d] mb-1">Consistency</p>
                    <p class="text-lg font-bold" style="color: {scoreColor(stats.consistency)}">{Math.round(stats.consistency)}<span class="text-xs text-[#9a8f7a] ml-1">%</span></p>
                </div>
            </div>
            ...
            <div class="bg-[#0a0809] rounded-lg p-4 border border-[#221c18]">
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        <p class="text-xs font-semibold text-[#f5a623] mb-1">Performance Insight</p>
                        <p class="text-sm text-[#9a8f7a]">
                            {#if stats.consistency >= 85}
                                Your {metricConfig[metric].label.toLowerCase()} is highly consistent (±{fmt(stats.stdDev, metricConfig[metric].decimals)}{metricConfig[metric].unit}). This excellent repeatability provides a stable foundation for improvement.
                            {:else if stats.consistency >= 70}
                                Your {metricConfig[metric].label.toLowerCase()} shows good consistency. Focus on tightening the spread (±{fmt(stats.stdDev, metricConfig[metric].decimals)}{metricConfig[metric].unit}) for more predictable results.
                            {:else if stats.consistency >= 55}
                                Your {metricConfig[metric].label.toLowerCase()} varies significantly (±{fmt(stats.stdDev, metricConfig[metric].decimals)}{metricConfig[metric].unit}). Work on consistent technique to reduce variation.
                            {:else}
                                High variation in {metricConfig[metric].label.toLowerCase()} (±{fmt(stats.stdDev, metricConfig[metric].decimals)}{metricConfig[metric].unit}). Focus on developing repeatable technique patterns.
                            {/if}
                            
                            {#if trend === 'improving'}
                                <span class="text-[#3de8c8]"> Positive trend detected — you're improving through the session!</span>
                            {:else if trend === 'declining'}
                                <span class="text-[#ffcc44]"> Values declining through session — may indicate fatigue.</span>
                            {/if}
                        </p>
                        ...
                        {#if annotatedPoints.some(p => p.isOutlier)}
                            ...
                            <p class="text-xs text-[#6b5f4d] mt-2 italic">
                                ⚠️ {outlierCount} outlier{outlierCount > 1 ? 's' : ''} detected — may indicate inconsistent conditions or technique variation.
                            </p>
                        {/if}
                    </div>
                </div>
