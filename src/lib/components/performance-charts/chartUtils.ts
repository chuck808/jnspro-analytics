...
export function buildPath(
	...
	const xScale = (timeS: number) =>
		padding + ((timeS - minX) / Math.max(maxX - minX, 0.001)) * (width - padding * 2);
	...
	return series
		.map(
			(p, index) =>
				`${index === 0 ? 'M' : 'L'} ${xScale(p.timeS).toFixed(2)} ${yScale(p.value).toFixed(2)}`
		)
		.join(' ');
}

export function normaliseToPrimaryScale(
	primary: SeriesPoint[],
