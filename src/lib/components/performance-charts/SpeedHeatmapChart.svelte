<script lang="ts">
  ...
  const maxIntensity = $derived(
    cells.length > 0 ? Math.max(...cells.map(c => c.intensity)) : 1
  );
      ...
      >
        {#each cells as cell, i}
          <div
            ...
            role="img"
            aria-label={cell.label ?? `Cell ${i + 1}: ${(cell.intensity * 100).toFixed(0)}% intensity`}
            title={cell.label ?? `${(cell.intensity * 100).toFixed(0)}% intensity${cell.value ? ` (${cell.value.toFixed(1)} ${unit})` : ''}`}
          ></div>
        {/each}
      </div>
    </div>
