<script lang="ts">
  ...
  import { buildPath, formatValue, getStats, normaliseToPrimaryScale, validSeries } from './chartUtils';
  ...
  const secondaryPath = $derived(secondaryData?.length
...
<section class:compact class="chart-card">
  ...
  {:else}
    <svg viewBox={`0 0 ${width} ${height ?? 220}`} role="img" aria-label={title} preserveAspectRatio="none">
      ...
      {#each yTicks as tick}
        ...
        <text x="16" y={tick.y + 4} class="tick-label" text-anchor="end">{formatValue(tick.value, 1)}</text>
      {/each}
      
      <!-- X-axis ticks and labels -->
      {#each xTicks as tick}
        <line x1={tick.x} y1={(height ?? 220) - 20} x2={tick.x} y2={(height ?? 220) - 28} class="tick" />
        <text x={tick.x} y={(height ?? 220) - 8} class="tick-label" text-anchor="middle">{formatValue(tick.value, 1)}</text>
      {/each}
      
      <!-- Main axes -->
      <line x1="24" y1={(height ?? 220) - 24} x2={width - 24} y2={(height ?? 220) - 24} class="axis" />
      <line x1="24" y1="24" x2="24" y2={(height ?? 220) - 24} class="axis" />
      
      <!-- Data area fills (below lines) -->
      {#if secondaryPath}
        <path d="{secondaryPath} L{width - 24},{(height ?? 220) - 24} L24,{(height ?? 220) - 24} Z" class="fill secondary-fill" />
      {/if}
      <path d="{path} L{width - 24},{(height ?? 220) - 24} L24,{(height ?? 220) - 24} Z" class="fill primary-fill" />
    ...
    {#if secondaryPath && secondaryLabel}
      <p class="legend"><span class="legend-primary"></span>{unit} <span class="legend-secondary"></span>{secondaryLabel}{secondaryUnit ? ` (${secondaryUnit})` : ''}</p>
    {/if}
  {/if}
</section>
...
<style>
  ...
  .legend-primary { background: var(--color-jns-amber, #f5a623); }
  .legend-secondary { background: var(--color-jns-mint, #3de8c8); }
  @media (max-width: 640px) {
    .chart-header { flex-direction: column; gap: 0.5rem; }
    .stats-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
    svg { min-height: 160px; }
  }
</style>
