<script lang="ts">
	...
	function getPriorityStyle(priority: Alert['priority']): string {
		switch (priority) {
			...
			case 'warning':
				return 'bg-[#f5a623]/10 border-[#f5a623]/30';
			case 'info':
				return 'bg-[#3de8c8]/10 border-[#3de8c8]/30';
		}
	}
...
<div class="rounded-lg border p-4 {priorityStyle}" role="alert">
	<div class="flex items-start gap-3">
		<!-- Icon -->
		<span class="flex-shrink-0 text-2xl">{alertIcon}</span>
		...
		<div class="min-w-0 flex-1">
			<div class="mb-1 flex items-start justify-between gap-2">
				...
				{#if alert.dismissible && onDismiss}
					<button
						onclick={onDismiss}
						class="-mt-1 -mr-1 rounded p-1 text-[#9a8f7a] transition-colors
