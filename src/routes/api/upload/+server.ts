...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	try {
		...
		try {
			...
			if (activeGoals && activeGoals.length > 0) {
				...
				for (const goal of activeGoals) {
					...
					if (sessionValue === null || sessionValue === undefined || previousValue === null)
						continue;
				...
				if (improvements.length > 0) {
					await processGoalImprovements(locals.supabase, improvements, ingestData.timestamp);
				}
			}
		} catch (goalErr) {
		...
		try {
			...
			const allGate = eligRuns.flatMap((r: any) =>
			...
			if (reactions.length >= 3) {
				...
				const std = Math.sqrt(
					reactions.map((v: number) => (v - mean) ** 2).reduce((a: number, b: number) => a + b, 0) /
						reactions.length
				);
