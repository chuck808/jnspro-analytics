...
async function sessionExport(
	...
	// Assemble rows
	const headers = [
		'rider_id', 'uci_category', 'age_at_session', 'sex',
		'height_cm', 'weight_kg', 'years_racing', 'dominant_leg',
		'participation_type', 'country',
		'session_date', 'track_surface', 'weather_condition', 'session_focus', 'ride_feel',
		'crank_length_mm', 'chainring_teeth', 'sprocket_teeth', 'gear_ratio', 'wheel_diameter_inches',
		'run_count', 'stats_eligible_runs',
		'best_reaction_ms', 'avg_reaction_ms', 'reaction_cv_pct',
		'best_max_g', 'avg_max_g',
		'best_peak_speed_ms', 'valid_speed_runs',
	];
	...
	for (const session of sessions ?? []) {
		...
		const wheelDiameter = bike?.custom_wheel_diameter_inches
			?? (bike?.rear_tire as any)?.diameter_inches
			?? null;

		const gearRatio = bike?.chainring_teeth && bike?.sprocket_teeth
...
async function runExport(
	...
	const { data: riders } = await admin
		.from('profiles')
		.select('id, country, participation_type, rider_profiles(height_cm, weight_kg, sex, years_racing, dominant_leg, date_of_birth, effective_from)')
		.eq('research_consent', true)
		.not('participation_type', 'is', null);
	...
	const headers = [
		'rider_id', 'uci_category', 'age_at_session', 'sex', 'height_cm', 'weight_kg',
		'years_racing', 'dominant_leg', 'participation_type', 'country',
		'session_date', 'run_number', 'is_stats_eligible',
		'track_surface', 'weather_condition', 'session_focus', 'ride_feel',
...
function buildSessionStats(runs: any[]): Record<string, {
	totalRuns: number; eligibleRuns: number;
	bestReactionMs: number | null; avgReactionMs: number | null; reactionCvPct: number | null;
	bestMaxG: number | null; avgMaxG: number | null;
	bestPeakSpeedMs: number | null; validSpeedRuns: number;
}> {
	...
	for (const run of runs) {
		...
		const tags: string[] = Array.isArray(run.tags) ? run.tags : [];
		const isEligible = !tags.includes('warmup') && !tags.includes('exclude');
		const gates = Array.isArray(run.gate_runs) ? run.gate_runs : (run.gate_runs ? [run.gate_runs] : []);
	...
	for (const [sessionId, { all, eligible }] of Object.entries(map) as any[]) {
		...
		const speeds    = eligible.filter((g: any) => g.analytics_valid).map((g: any) => g.peak_speed_ms).filter((v: any) => v != null);

		const avgReaction = reactions.length ? reactions.reduce((a: number, b: number) => a + b, 0) / reactions.length : null;
		const reactionCv  = reactions.length >= 3 && avgReaction
			? (Math.sqrt(reactions.map((v: number) => (v - avgReaction) ** 2).reduce((a: number, b: number) => a + b, 0) / reactions.length) / avgReaction) * 100
			: null;
		...
		result[sessionId] = {
			...
			avgMaxG:         gForces.length ? gForces.reduce((a: number, b: number) => a + b, 0) / gForces.length : null,
...
function buildRiderMap(riders: any[]): Record<string, RiderContext> {
	...
	for (const rider of riders) {
		...
		const profiles: any[] = Array.isArray(rider.rider_profiles)
			...
			: (rider.rider_profiles ? [rider.rider_profiles] : []);
