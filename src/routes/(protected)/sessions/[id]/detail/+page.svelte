<script lang="ts">
    ...
    import { computeSessionStability, computeGForceStability, getStabilityInsight } from '$lib/performance-engine/sessionAnalysis';
    ...
    // ── Comparison runs ────────────────────────────────────────────────────────
    let comparisonRuns = $derived(performanceAnalysis.runs.map((r: any, idx: number) => ({
    ...
    let sessionStability = $derived.by(() => {
        ...
        return computeSessionStability(data.runs.map((r: any) => ({
    ...
    let stabilityInsight = $derived.by(() => {
        if (sessionStability.length < 2 || currentStability === null) return 'Single run — no comparison available';
        return getStabilityInsight(currentStability, sessionStability);
    });

    // ── Drill-down data ────────────────────────────────────────────────────────
    let drillDownData = $derived(chartData.map((value: number, idx: number) => ({
    ...
    const priorityColor: Record<string, string> = {
        high: '#ff4444', medium: '#f5a623', low: '#9a8f7a'
    };
</script>
    ...
    {#if selectedRun && selectedGate}
        ...
        {#if sessionStability.length > 1}
            <div class="bg-[#131010] border border-[#221c18] rounded-xl p-5">
                <h3 class="text-sm font-semibold text-[#f0ece4] mb-1">G-Force Stability — First 0.5s</h3>
                <p class="text-xs text-[#6b5f4d] mb-4">
                    Average G-force applied in the first 500ms of each run.
                    Higher = more aggressive initial stroke · Consistency = repeatable technique.
                </p>
                <div class="flex items-end gap-2 h-24" role="img" aria-label="G-force stability comparison across runs">
                    {#each sessionStability as s}
                        ...
                        <div class="flex-1 flex flex-col items-center gap-1">
                            ...
                            <div class="w-full rounded-t transition-all"
                                 style="height:{pct}%; min-height:4px; background:{s.isBest ? '#f5a623' : s.runNumber === selectedRun.run_number ? '#f5a62380' : '#221c18'}">
                            </div>
                ...
                <div class="mt-4 flex items-start gap-3 p-3 bg-[#0a0809] rounded-lg border border-[#221c18]">
                    <svg class="w-4 h-4 text-[#f5a623] flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                    <p class="text-xs text-[#9a8f7a] leading-relaxed">{stabilityInsight}</p>
                </div>
                <p class="text-xs text-[#6b5f4d] mt-2">Amber = best · Highlighted = selected run</p>
            </div>
        {/if}
        ...
        {#if !totalMassKg}
            <div class="bg-[#131010] border border-[#f5a623]/20 rounded-xl p-4 text-sm">
                <p class="text-[#f5a623] font-medium mb-1">Power analytics locked</p>
                <p class="text-[#9a8f7a]">Add your weight and bike weight in
                    <a href="/profile" class="text-[#f5a623] hover:underline focus:outline-none focus:ring-1 focus:ring-[#f5a623] rounded">your profile</a>
                    to unlock power and impulse analysis.</p>
            </div>
        {/if}

        {#if performanceAnalysis.hasCalibrationWarning}
            <div class="bg-[#0a0809] border border-[#ff4444]/20 rounded-xl p-4">
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        <p class="text-sm font-semibold text-[#ff4444] mb-1">Calibration Warning</p>
                        <p class="text-xs text-[#9a8f7a] mb-2">Some physics values appear outside normal ranges. Power and speed estimates may be unreliable.</p>
                            ...
                            <div class="space-y-1">
                                {#each performanceAnalysis.diagnostics.filter((d: any) => d.severity === 'error') as diag}
                                    <p class="text-xs text-[#9a8f7a]">
                                        <span class="text-[#ff4444]">•</span> {diag.message}
                                        {#if diag.suggestion}<span class="text-[#6b5f4d]">— {diag.suggestion}</span>{/if}
                                    </p>
                                {/each}
                            </div>
                        {/if}
        ...
        {#if weaknesses.length > 0}
            <div class="bg-[#131010] border border-[#221c18] rounded-xl p-5">
                <h3 class="text-sm font-semibold text-[#f0ece4] mb-1">Areas for Improvement</h3>
                <p class="text-xs text-[#6b5f4d] mb-4">Metrics scoring below threshold for {riderLevel ?? 'intermediate'} level</p>
                <div class="space-y-3">
                        ...
                        <div class="bg-[#0a0809] rounded-lg p-4 border border-[#221c18]">
                            ...
                            <ul class="space-y-1">
                                {#each w.advice as tip}
                                    <li class="text-xs text-[#9a8f7a] flex gap-2">
                                        <span class="text-[#f5a623] flex-shrink-0" aria-hidden="true">→</span>{tip}
                                    </li>
                                {/each}
                            </ul>
                        </div>
        ...
        {#if recommendations.length > 0}
            <div class="bg-[#131010] border border-[#221c18] rounded-xl p-5">
                <h3 class="text-sm font-semibold text-[#f0ece4] mb-1">💡 Recommendations</h3>
                <p class="text-xs text-[#6b5f4d] mb-4">Prioritised actions based on this run's data</p>
                <div class="space-y-3">
                    {#each recommendations as rec}
                        <div class="bg-[#0a0809] rounded-lg p-4 border border-[#221c18]">
                            <div class="flex items-start gap-3">
                                <span class="text-xs font-bold px-2 py-0.5 rounded flex-shrink-0 mt-0.5"
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-[#9a8f7a] mt-0.5">{rec.message}</p>
                                </div>
                            </div>
                        </div>
                    {/each}
            ...
            <div class="flex items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-lg bg-[#3de8c8]/10 flex items-center justify-center group-hover:bg-[#3de8c8]/20 transition-colors">
                        <svg class="w-5 h-5 text-[#3de8c8]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                  d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/>
                        </svg>
                    </div>
                    <div>
                        <h3 class="text-sm font-semibold text-[#f0ece4] group-hover:text-[#3de8c8] transition-colors mb-0.5">📊 Compare to Previous Session</h3>
                        <p class="text-xs text-[#6b5f4d]">See how your performance changed session-to-session</p>
                    </div>
                </div>
                <svg class="w-5 h-5 text-[#6b5f4d] group-hover:text-[#3de8c8] transition-colors"
    ...
    <button
        ...
        <div class="flex items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-sm font-semibold text-[#f0ece4] group-hover:text-[#f5a623] transition-colors mb-0.5">📄 Generate Session Report</h3>
                    <p class="text-xs text-[#6b5f4d]">Create professional coaching report with customizable options</p>
                </div>
            </div>
            <svg class="w-5 h-5 text-[#6b5f4d] group-hover:text-[#f5a623] transition-colors"
                 fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
            </svg>
        </div>
