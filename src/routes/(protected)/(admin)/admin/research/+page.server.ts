	...
	const [consentedResult, totalResult, participationResult, sessionCountResult] = await Promise.all(
		[
			...
			admin
				.from('profiles')
				.select(
					'id, country, participation_type, research_consent_at, rider_profiles(sex, height_cm, weight_kg, date_of_birth, years_racing, dominant_leg)'
				)
			...
			admin.from('profiles').select('participation_type').not('participation_type', 'is', null),
