<script lang="ts">
	...
	const stats = $derived.by(() => {
		...
		const byStatus = (data.feedback as FeedbackItem[]).reduce(
			(acc, f) => {
...
<div class="space-y-6">
	...
	<div class="flex items-center justify-between">
		<div>
			<h2 class="themed-text-primary text-lg font-bold">Feedback Management</h2>
	...
	<div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
		<div class="themed-card rounded-xl p-5">
			<div class="themed-text-secondary mb-1 text-xs tracking-wider uppercase">Total</div>
			<div class="themed-text-primary text-3xl font-bold">{stats.total}</div>
		</div>
		<div class="themed-card rounded-xl p-5">
			<div class="themed-text-secondary mb-1 text-xs tracking-wider uppercase">New</div>
			<div class="text-3xl font-bold" style="color: {statusColors.new}">{stats.new}</div>
		</div>
		<div class="themed-card rounded-xl p-5">
			<div class="themed-text-secondary mb-1 text-xs tracking-wider uppercase">In Progress</div>
			<div class="text-3xl font-bold" style="color: {statusColors.in_progress}">
				{stats.in_progress}
			</div>
		</div>
		<div class="themed-card rounded-xl p-5">
			<div class="themed-text-secondary mb-1 text-xs tracking-wider uppercase">Resolved</div>
	...
	<div class="filters flex gap-4">
		<div class="max-w-xs flex-1">
			<label for="type-filter" class="themed-text-secondary mb-1.5 block text-sm font-medium"
				>Type</label
			>
			<select
				id="type-filter"
				bind:value={filterType}
				class="themed-text-primary w-full rounded-lg border border-[color:var(--border)] bg-[color:var(--background)] px-4 py-2.5
                           text-sm transition-colors focus:border-[color:var(--accent)] focus:ring-1 focus:ring-[color:var(--accent)] focus:outline-none"
			>
		...
		<div class="max-w-xs flex-1">
			<label for="status-filter" class="mb-1.5 block text-sm font-medium text-[#9a8f7a]"
				>Status</label
			>
			<select
				id="status-filter"
				...
				class="w-full rounded-lg border border-[#221c18] bg-[#0a0809] px-4 py-2.5 text-sm
                           text-[#f0ece4] transition-colors focus:border-[#f5a623] focus:ring-1 focus:ring-[#f5a623] focus:outline-none"
			>
		...
		{#each filteredFeedback as feedback (feedback.id)}
			<button class="feedback-item" onclick={() => openDetail(feedback)}>
				<div class="fi-header">
					...
					<div
						class="fi-status"
						style="background: {statusColors[feedback.status ?? 'new']}20; color: {statusColors[
							feedback.status ?? 'new'
						]}"
					>
					...
					<div class="detail-type flex items-center gap-2">
						...
						>
							<path
								...
								stroke-linejoin="round"
								stroke-width="2"
								d={typeIconPaths[selectedFeedback.type]}
							/>
						</svg>
			...
			<div class="modal-body">
				...
				<div class="detail-section">
					...
					<p class="detail-text">
						...
						{formatDate(selectedFeedback.created_at)}<br />
