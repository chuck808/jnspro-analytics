...
export const actions: Actions = {
	...
	saveProfile: async ({ request, locals: { supabase } }) => {
		...
		const years_racing = form.get('years_racing')
			? parseInt(form.get('years_racing') as string)
			: null;
	...
	saveBike: async ({ request, locals: { supabase } }) => {
		...
		const weight_kg = form.get('weight_kg') ? parseFloat(form.get('weight_kg') as string) : null;
		const crank_length_mm = form.get('crank_length_mm')
			? parseInt(form.get('crank_length_mm') as string)
			: null;
		...
		const front_tire_id = form.get('front_tire_id')
			? parseInt(form.get('front_tire_id') as string)
			: null;
		const rear_tire_id = form.get('rear_tire_id')
			? parseInt(form.get('rear_tire_id') as string)
			: null;
